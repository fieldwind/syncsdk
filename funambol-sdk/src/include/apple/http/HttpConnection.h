...
private:
    ...
    /// The connection type: is updated at every open() call, checking the network properties.
    static HttpConnectionType connectionType;
...
public:
    ...
    /**
     * Returns true if the current connection is a Wifi, false if not or unknown.
     * The connection type is updated at every open() call, checking the network properties.
     */
    static bool isWifiConnection();
    static bool isWifiConnection(const URL& requestUrl);
    static bool existsConnection(const URL& url); 
    static bool onlyGPRSConnection(const URL& url);
...
private:
    ...
    int  sendRequest(CFReadStreamRef requestBodyStream, OutputStream& os, size_t requestPayloadSize);
