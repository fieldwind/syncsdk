 ...
 * localTime means that if it is false, the time is converted in UTC and a Z is put at the end
 */
...
/**
 * It returns the timezone on the current calendar
 */
...
/**
 * Formats a time difference in seconds in the format +|-hhmm
 */
NSString* formatDifference(int difference);

/**
 * Formats the difference between a date and GMT in a given timezone
 * in the format +|-hhmm
 */
NSString* formatDifferenceForDate(NSDate* date, NSTimeZone* tz);

/**
 * Returns the offset of a given timezone from GMT in seconds
 */
NSInteger offsetFromGMT(NSTimeZone* tz);

/**
 * Returns YES if the given time zone observers daylight savings
 */
BOOL observesDST(NSTimeZone* tz);


/**
 * Returns a date with an offset of years from now
 */
NSDate* yearsFromNow(int years);

/**
 * Returns a date with an offset of months from now
 */
