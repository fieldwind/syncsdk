...
protected:
    ...
    int updateItemStatus(SyncItem& item);
    ...
    */
	...
	int updateInCache(SyncItem& item);
	
	void logSuccess(const char* operation, const char* name, const char* key, int result);
	void logFailure(const char* operation, const char* name, const char* key, int result);
	const char* opName(int syncStatus);
    ...
    */
    ...
    virtual bool checkAbortSync() { return false; }

	KeyValueStore* getCache();
...
public:
    ...
    * @return       the signature (as string) of the selected item (emtpy string
    *               if the key is void)
    */    
    ...
    */
     ...
     * or leave. Currently it considers non error codes the values in the range
     * between 200 <= code < 300 and code = 418. The remaining code are errors.
     *
    ...
    virtual void fireClientTotalNumber(int number);
     ...
     /**
      * return true if there is a local modification
      */
    virtual bool isThereLocalModification();
    
    /**
     * Gives the syncsource the possibility to check a key against it's item
     * store to decide if an item has really been deleted (needed for calendar
     * sync)
     */
    virtual bool isReallyDeleted(StringBuffer& key) { return true; }
};
