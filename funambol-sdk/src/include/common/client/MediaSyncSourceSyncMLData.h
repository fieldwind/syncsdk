...
#ifndef MEDIASOURCESYNCMLDATASYNC_H_
#define MEDIASOURCESYNCMLDATASYNC_H_
...
#include "client/MediaSyncSource.h"     // the "new" class, for MediaSyncSourceParams
...
/**
 * -------------------------------------------------------------------------------------
 * NOTE: this class is the OLD version of MediaSyncSource, used to sync media items
 * only via syncML (no http upload/download), so the file's data is exchanged via SyncML. 
 * This was the implementation of MediaSyncSource class in v8.5, used for backward compatibility.
 * See MediaSyncSource.h for the new implementation since v8.7.
 * -------------------------------------------------------------------------------------
 ...
 */
class MediaSyncSourceSyncMLData : public FileSyncSource
{
 
public:
    MediaSyncSourceSyncMLData(const WCHAR* wname,
                   AbstractSyncSourceConfig* sc,
                   const StringBuffer& aDir, 
                   MediaSyncSourceParams mediaParams);

    ~MediaSyncSourceSyncMLData();
...
#endif /*MEDIASOURCESYNCMLDATASYNC_H_*/
