...
typedef enum ESapiSyncManagerError {
    ...
    ESSMInvalidItemPermanentlyRefused
...
public:
    ...
    /**
     * Checks if there's at least one item changed locally.
     * If it's the 1st sync (ALL items) checks the existence of at least 1 item locally.
     * @return true  if a local change is detected, false if not
     */
    ...
    bool areThereModifications(bool add, bool mod, bool del, const char* clientOrServer = NULL);
...
protected:
     ...
     *                   COMMAND_ADD, COMMAND_REPLACE
     * From v11 it handles rename as well
    ...
    /**
     * V11: Deletes one single local item to the server.
     * Called by upload() for each item.
     *
     * @return  the delete result, 0 if no error
     */
    int deleteItem(SapiSyncItemInfo& sapiItem);
    ...
    /**
     * Based on the errorCode passed, returns true if the download of next 
     * items should stop. Returns false if it's a non-blocking error and
     * the download can continue.
     */
    bool shouldStopAllDownloads(const int errorCode);

    /**
     * Based on the errorCode passed, returns true if the upload of next 
     * items should stop. Returns false if it's a non-blocking error and
     * the upload can continue.
     */
    bool shouldStopAllUploads(const int errorCode);
