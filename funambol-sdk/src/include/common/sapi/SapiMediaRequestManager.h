...
typedef enum ESapiMediaRequestStatus 
{
    ...
    // error for item that must be blacked list on the client because the server cannot add them
    ESMRInvalidItemPermanentlyRefused,
    ESMRNetworkOrServerDown     // used in the first login phase when try to log if some errors
...
typedef enum ESapiMediaSourceType
{
    ...
    ESapiMediaSourceMusic,
    ESapiMediaSourceContacts
} SapiMediaSourceType;
...
class SapiMediaRequestManager
{
    ...
    public:
        ...
        virtual ESapiMediaRequestStatus deleteItem(SapiSyncItemInfo* item);
        virtual ESapiMediaRequestStatus renameItem(SapiSyncItemInfo* item, time_t* lastUpdate);

        // SAPI session set up methods
        // virtual ESapiMediaRequestStatus login(const char* device_id, time_t* serverTime = NULL); // (substituted with new function with more parameters)
        virtual ESapiMediaRequestStatus login(const char* device_id, time_t* serverTime = NULL, unsigned long *expiretime = NULL,
                                              StringMap* sourcesStringMap = NULL, StringMap* propertyStringMap = NULL);

        virtual ESapiMediaRequestStatus logout();

        virtual ESapiMediaRequestStatus getServerInformation(StringMap* responseParams);
