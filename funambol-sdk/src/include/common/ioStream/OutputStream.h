...
public:
    virtual void written(int64_t size) {}
    virtual void movedToPosition(int64_t position) {}
};
...
protected:
    ...
    OutputStreamObserver* observer;


public:

    /// Constructor
    OutputStream() : bytesWritten(0), observer(NULL) {}
    ...
    virtual int64_t write(const void* buffer, int64_t size) {
        int64_t res = writeBuffer(buffer, size);
        if (observer != NULL && res >= 0) {
            observer->written(res);
        }
        return res;
    }
    
    virtual int64_t writeBuffer(const void* buffer, int64_t size) = 0;
    ...
    void setObserver(OutputStreamObserver* observer) {
