...
// incoming stream from server. It is expressed in byte
//
...
#define TA_PropertyDeviceId                     "X-deviceid"

#define TA_Property_X_3GPP_Intended_Identity        "X-3GPP-Intended-Identity"
#define TA_Property_Authorization                    "Authorization"
...
class TransportAgent {
    ...
    StringBuffer phoneidentify;
    StringBuffer tokenauth;
    ...
    virtual const char*  getPhoneidentify() ;
    virtual void setPhoneidentify(const char*  phoneidentify);
    
    virtual const char*  getTokenauth() ;
    virtual void setTokenauth(const char*  tokenauth);
        ...
        /**
        * Method to remove sensible data from the syncml message. At the moment removes the vcard and vcalendar in the 
        * syncml message and substitutes with *****. 
        * It should be used for logging pourpose sending and receving data
        * @param msg: the syncml message that has to be transferred to the server. It will be untouched
        * @return the syncml message without explicit vcard and vcalendar.
        *
        */
        std::string removeSensibleData(const char* msg);
