...
#include "ioStream/InputStream.h"
...
#define HTTP_HEADER_X_FUNAMBOL_ID                   "X-funambol-id"      // sapi media add
#define HTTP_HEADER_X_DEVICE_ID                     "X-deviceid"         

/// Max chunk size for http requests/responses if not explicitly set [bytes]
#define DEFAULT_CHUNK_SIZE           10000

/// HTTP timeout to send a request / receive a response from the Server if not explicitly set [seconds]
#define DEFAULT_HTTP_TIMEOUT            120

class HttpConnectionUploadObserver {

public:
    HttpConnectionUploadObserver() : initialProgress(0),
                                     progress(0)
    {
    }
    
    virtual void uploadProgress(size_t size) { progress = initialProgress + size; }
    virtual void setStartProgress(size_t start) { initialProgress = start; }
    
protected:
    size_t initialProgress;
    size_t progress;
};
...
public:
    ...
    enum ConnectionStatus {
        ...
        StatusTimeoutError    = -7,
        StatusStreamWritingError = -8,
		StatusStreamReadingError = -9
    };
    ...
    AbstractHttpConnection(const char* ua) : userAgent(ua)  {
        ...
        responseChunkSize   = DEFAULT_CHUNK_SIZE;
        SSLVerifyServer     = true;
        SSLVerifyHost       = true;
        compression_enabled = false;
        keepalive           = false;
        requestTimeout      = DEFAULT_HTTP_TIMEOUT;
        responseTimeout     = DEFAULT_HTTP_TIMEOUT;
        uploadObserver      = NULL;
    }
     ...
     */
    virtual int open(const URL& url, RequestMethod method = MethodPost, bool log_request=true) = 0;
     ...
     */
    virtual int request(InputStream& data, OutputStream& response, bool log_request=true) = 0;
	...
	virtual void setRequestHeaders(StringMap headers) {
    ...
    virtual StringMap getRequestHeaders() {
        return requestHeaders;
    }

	virtual StringMap getResponseHeaders() {
    ...
    /// Sets the max chunk size, for HTTP requests (outgoing data is chunked)
    void setRequestChunkSize(const int size) { requestChunkSize = size; }
    int  getRequestChunkSize()               { return requestChunkSize; }

    /// Sets the max chunk size, for HTTP responses (incoming data is chunked)
    void setResponseChunkSize(const int size) { responseChunkSize = size; }
    int  getResponseChunkSize()               { return responseChunkSize; }
	 ...
	 */
    virtual void setRequestTimeout(unsigned int t) { requestTimeout = t; }

	/// Returns the connection request timeout
    virtual unsigned int getRequestTimeout() const { return requestTimeout; }

	/**
	 * Sets the connection response timeout
	 * @param t the new timeout in seconds
	 */
    virtual void setResponseTimeout(unsigned int t) { responseTimeout = t; }

	/// Returns the connection response timeout
    virtual unsigned int getResponseTimeout() const { return responseTimeout; }
    ...
    void setUploadObserver(HttpConnectionUploadObserver* observer) {
...
protected:
    ...
    int64_t requestChunkSize;

    /// Max chunk size for http responses, in bytes
    int responseChunkSize;

	Proxy proxy;

    /// Timeout to drop connection while sending http requests.
	unsigned int requestTimeout;

    /// Timeout to drop connection while receiving http responses.
    ...
    /**
     * Returns true if the code passed is an HTTP status code (HTTP 1.1 specs).
     * 2xx      -> Success (200 = OK, 206 = partial GET,...)
     * 4xx, 5xx -> Error
     * 3xx      -> Redirection (no error)
     */
    virtual bool isErrorStatus(int httpStatusCode) {
