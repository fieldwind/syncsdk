...
typedef enum {
    ...
    EMSUInvalidCountryCode,                     // Invalid country code for the phone number
    EMSUForbidden,                              // User not authorized to login (e.g. user not active
    EMSUPaymentRequired,
    EMSUNetworkError,                           // Network error (timeouts, etc..)
    EMSUHTTPFunctionalityNotSupported,          // It gets a 501 error from the server (check fields on server not supporting this sapi)
    EMSUGenericError
}  EMSUStatusCode;
...
private:
    ...
    static const char* sapiSignUpUrlFmtNoCaptcha;
    static const char* sapiCheckValidityFmt;
...
public:
     ...
     */
    EMSUStatusCode signup(const char* username, const char* password, const char* phoneNumber,
                          const char *token, MSUDeviceInfo* deviceInfo,
                          std::string& serverErrorMsg);
    EMSUStatusCode signupNoCaptcha(const char* username, const char* password,
                                   MSUDeviceInfo* deviceInfo,
                                   std::string& serverErrorMsg);

    
    /**
     * Check the right format of the fields: they must contain only some char, no space and so on...
     * 
     * @param username  the field to check
     * @param password  the field to check
     * @return          the result of the check
     */
    EMSUStatusCode checkFieldsValidity(const char* username, const char* password, const char* language,
