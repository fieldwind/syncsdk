...
public:
    CTPThread(CTPService* ctpService_);
    ~CTPThread();
    void run();
...
public:
    ReceiverThread(CTPService* ctpService_);
    ~ReceiverThread();
    void run();
    int32_t getErrorCode() { return errorCode; }
...
public:
    HeartbeatThread(CTPService* ctpService_);
    ~HeartbeatThread();
    void run();
    int32_t getErrorCode() { return errorCode; }
    void softTerminate();
...
public:
    CmdTimeoutThread(CTPService* ctpService_);
    ~CmdTimeoutThread();
    void run();
    void softTerminate();
...
private:
    ...
    // client DMT to manage AccessConfig
    DMTClientConfig* clientConfig;
    
    pthread_mutex_t ctpSocketMtx;
...
protected:
    ...
    // Constructor
    CTPService(DMTClientConfig* clientConfig_);


public:

    // Method to get the sole instance of CTPService
    static CTPService* getInstance(DMTClientConfig* clientConfig_);
