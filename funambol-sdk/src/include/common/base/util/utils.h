...
/// Converts a string into a long long (64bit) like _atoi64()
...
/**
 * Just mapped to stdio rename().
 * On Windows platform, using the _wrename() in order to correctly
...
/**
 * Returns the total number of bytes and free bytes on a disk.
 * @param path       a directory on the disk.
 ...
 * @param totalBytes [OUT] A pointer to a variable that receives the total number of bytes on disk
 * @param freeBytes  [OUT] A pointer to a variable that receives the total number of free bytes on disk
 * @return           0 if no error, ERR_FILE_SYSTEM if the path is not available, a value != 0 otherwise
 */
...
/**
 * Just mapped to stdio stat().
 * On Windows platform, using the _wstat() in order to correctly
 * manage file names with special chars.
 */
...
* @return  true if file(s) removed successfully
*/
...
/**
 * Triggers a sleep of this thread for a certain amount of milliseconds.
 */
...
/**
* Read the files into a directory in recursive way it recursive is true, applying
* a filter on directories names or patterns to be skipped.
* Returns an array list of files with full path dirname/filename, dirname/dir1/fileinside
*
* @param dirname the dir to look into
* @param recursive if true it looks inside the inner dirs
* @param filterPatterns a std::string vector of name patterns to be filtered
* @param a std::map containing all the scanned folders absolute path 
*
* @return an ArrayList of Stringbuffer containing the file names 
*/

ArrayList readFilesInDirRecursive(const char* dirname, bool recursive, 
                const std::vector<std::string>& filterPatterns, std::map<std::string, bool>& readFoldersMap);
...
/**
* Checks a string value against a vector of string for matches (with limited
* support for globbing)
* Returns true if there was a match
*
* @param value - a std::string value to use as a search pattern
* @param filterValuesList - a std::vector of string to match against with pattern
*
...
/**
* 
* Splits a std::string into tokens separated by a delimiter character  
* filling a vector 
*
* @param s     - a string to be tokenized
* @param delim - a delimiter character used to tokenize the string
* @param elems - a std::vector reference filled with the tokens. 
*                Vector is not reset: if not empty, new elements found are just pushed to the end of the vector. 
*
...
/**
* 
* Splices a std::vector into std::string eventually separated by a delimiter character  
* 
* @param elts a std::string vector
* @param delim a std::string used in vector elts splicing
*
* @return a std::string with vector elemets spliced
*/
std::string spliceVectorToStringByDelim(const std::vector<std::string> &elts, const std::string& delim = "");

/**
 * return appropiate struct stat element for file creation 
 * @param st - a struct stat pointer
 * 
 * @return st_bitrhdate on system where is available, otherwise st_ctime
 */
