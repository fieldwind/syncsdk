...
class SyncManagerConfig : public AbstractSyncConfig {
    protected:
        ...
        // MHConfig groups the config params related to MH media sync
        MHConfig mhConfig;
    
        // sapiConfig groups the config params related to SAPI media sync v10 (back comp)
        // TODO: remove when v10 back comp is no more needed
        SapiConfig sapiConfig;

        // Array of source's configs
        ...
        unsigned long installationTimestamp;
        
        std::string sessionId;
        time_t sessionIdSetTime;
        time_t sessionIdExpirationTime;
        std::string validationKey;

        bool sslServerVerifier;
    ...
    public:
        ...
        // Get/set MHConfig for MH config params
        virtual const MHConfig& getMHConfig() const { return mhConfig; }
        virtual       MHConfig& getMHConfig()       { return mhConfig; }
        virtual void  setMHConfig(MHConfig& sc)     { mhConfig.assign(sc); }
    
        // Get/set sapiConfig for SAPI config params
        virtual const SapiConfig& getSapiConfig() const { return sapiConfig; }
        virtual       SapiConfig& getSapiConfig()       { return sapiConfig; }
        virtual void  setSapiConfig(SapiConfig& sc)     { sapiConfig.assign(sc); }
        ...
        /**
         * Resets the lastGlobalError with default value
         */
        void resetLastError();
        ...
        virtual bool  getSSLVerifyServer() const { return sslServerVerifier; }
        virtual void  setSSLVerifyServer(bool val) { sslServerVerifier = val; }
    virtual const char*  getPhoneidentify() const { return getAccessConfig().getPhoneidentify(); }
    virtual const char*  getTokenauth() const { return getAccessConfig().getTokenauth(); }
        ...
        virtual unsigned long getInstallationTimestamp() {
            return installationTimestamp;
        }
        virtual void setInstallationTimestamp(unsigned long val) {
            installationTimestamp = val;
        }

        virtual const char* getSessionId() const                       { return sessionId.c_str(); }
        virtual void setSessionId(const char* sessionId_)              { sessionId = sessionId_; }
         
        virtual time_t getSessionIdSetTime() const                     { return sessionIdSetTime; }
        virtual void setSessionIdSetTime(time_t setTime)               { sessionIdSetTime = setTime; }
        
        virtual time_t getSessionIdExpirationTime() const              { return sessionIdExpirationTime; }
        virtual void setSessionIdExpirationTime(time_t expirationTime) { sessionIdExpirationTime = expirationTime; }  

        virtual const char* getValidationKey() const                   { return validationKey.c_str(); }
        virtual void setValidationKey(const char* validationKey_)      { validationKey = validationKey_; }
    
        //
        // OAuth2 support
        //
        void setAuthenticationType(const char* authType)        { getAccessConfig().setAuthenticationType(authType); }
        void setClientAuthType(const char* authType)            { getAccessConfig().setClientAuthType(authType);     }
        const char* getAuthenticationType() const               { return getAccessConfig().getAuthenticationType();  }

    
        void setOAuth2AccessToken(const char* accessToken)   { getAccessConfig().setOAuth2AccessToken(accessToken);    }
        void setOAuth2AccessTokenSetTime(time_t setTime)     { getAccessConfig().setOAuth2AccessTokenSetTime(setTime); }
        void setOAuth2RefreshToken(const char* refreshToken) { getAccessConfig().setOAuth2RefreshToken(refreshToken);  }
        void setOAuth2ExpiresIn(const char* expiresIn)       { getAccessConfig().setOAuth2ExpiresIn(expiresIn);        }
        void setOAuth2Scope(const char* scope)               { getAccessConfig().setOAuth2Scope(scope);                }
        void setOAuth2ClientType(const char* clientType)     { getAccessConfig().setOAuth2ClientType(clientType);      }
    
        const char* getOAuth2AccessToken(void)   const       { return getAccessConfig().getOAuth2AccessToken();  }
        time_t getOAuth2AccessTokenSetTime(void) const       { return getAccessConfig().getOAuth2AccessTokenSetTime();   }
        const char* getOAuth2RefreshToken(void)  const       { return getAccessConfig().getOAuth2RefreshToken(); }
        const char* getOAuth2ExpiresIn(void)     const       { return getAccessConfig().getOAuth2ExpiresIn();    }
        const char* getOAuth2Scope(void)         const       { return getAccessConfig().getOAuth2Scope();        }
        const char* getOAuth2ClientType(void)    const       { return getAccessConfig().getOAuth2ClientType();   }
  
        //
        // Wassup support
        //
        void setWassupAuthToken(const std::string& authToken)  { getAccessConfig().setWassupAuthToken(authToken); }
        const char* getWassupAuthToken(void)   const          { return getAccessConfig().getWassupAuthToken();   }
     
        //
        // Proxy getters/setters for MHConfig params
        //
        virtual int getMHRequestTimeout()    { return getMHConfig().getRequestTimeout();    }
        virtual int getMHResponseTimeout()   { return getMHConfig().getResponseTimeout();   }
        virtual int getMHUploadChunkSize()   { return getMHConfig().getUploadChunkSize();   }
        virtual int getMHDownloadChunkSize() { return getMHConfig().getDownloadChunkSize(); }
        virtual int getMHMaxRetriesOnError() { return getMHConfig().getMaxRetriesOnError(); }
        virtual long getMHSleepTimeOnRetry() { return getMHConfig().getSleepTimeOnRetry();  }
        virtual bool getMHResetStreamOnRetry() { return getMHConfig().getResetStreamOnRetry(); }
        virtual long getMHMinDataSizeOnRetry() { return getMHConfig().getMinDataSizeOnRetry();  }

        virtual void setMHRequestTimeout   (const int v) { getMHConfig().setRequestTimeout(v);    }
        virtual void setMHResponseTimeout  (const int v) { getMHConfig().setResponseTimeout(v);   }
        virtual void setMHUploadChunkSize  (const int v) { getMHConfig().setUploadChunkSize(v);   }
        virtual void setMHDownloadChunkSize(const int v) { getMHConfig().setDownloadChunkSize(v); }
        virtual void setMHMaxRetriesOnError(const int v) { getMHConfig().setMaxRetriesOnError(v); }
        virtual void setMHSleepTimeOnRetry(const long v) { getMHConfig().setSleepTimeOnRetry(v);  }
        virtual void setMHResetStreamOnRetry (bool v) { getMHConfig().setResetStreamOnRetry(v);  }
        virtual void setMHMinDataSizeOnRetry(const long v) { getMHConfig().setMinDataSizeOnRetry(v);  }
    
        //
        // Proxy getters/setters for SapiConfig params (v10 backward compatibility)
        //
        virtual int getSapiRequestTimeout()      { return getSapiConfig().getRequestTimeout();    }
        virtual int getSapiResponseTimeout()     { return getSapiConfig().getResponseTimeout();   }
        virtual int getSapiUploadChunkSize()     { return getSapiConfig().getUploadChunkSize();   }
        virtual int getSapiDownloadChunkSize()   { return getSapiConfig().getDownloadChunkSize(); }
        virtual int getSapiMaxRetriesOnError()   { return getSapiConfig().getMaxRetriesOnError(); }
        virtual long getSapiSleepTimeOnRetry()   { return getSapiConfig().getSleepTimeOnRetry();  }
        virtual bool getSapiResetStreamOnRetry() { return getSapiConfig().getResetStreamOnRetry();}
        virtual long getSapiMinDataSizeOnRetry() { return getSapiConfig().getMinDataSizeOnRetry();}
        virtual const char* getSapiVersion()     { return getSapiConfig().getSapiVersion();       }

        virtual void setSapiRequestTimeout   (const int v)   { getSapiConfig().setRequestTimeout(v);    }
        virtual void setSapiResponseTimeout  (const int v)   { getSapiConfig().setResponseTimeout(v);   }
        virtual void setSapiUploadChunkSize  (const int v)   { getSapiConfig().setUploadChunkSize(v);   }
        virtual void setSapiDownloadChunkSize(const int v)   { getSapiConfig().setDownloadChunkSize(v); }
        virtual void setSapiMaxRetriesOnError(const int v)   { getSapiConfig().setMaxRetriesOnError(v); }
        virtual void setSapiSleepTimeOnRetry(const long v)   { getSapiConfig().setSleepTimeOnRetry(v);  }
        virtual void setSapiResetStreamOnRetry (bool v)      { getSapiConfig().setResetStreamOnRetry(v);}
        virtual void setSapiMinDataSizeOnRetry(const long v) { getSapiConfig().setMinDataSizeOnRetry(v);}
        virtual void setSapiVersion(const char* v)           { getSapiConfig().setSapiVersion(v);       }
};
