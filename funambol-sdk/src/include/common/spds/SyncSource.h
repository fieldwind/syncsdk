...
public:
    ...
    */
    ...
    virtual void updateItemInfo(SyncItem& item, int index)=0;
    virtual int updateItemStatus(SyncItem& item)=0;
    virtual void saveAddressBook()=0;
    ...
    virtual void applyItems(ArrayList& items);           
    
    /**
     * Return the number of elements in this source (-1) if unknown
     */
    virtual long getTotalNumberOfLocalItems();
    
    /**
     * Return true if there are local changes in this source since last modification time
     * (from endSyncTime property)
     */
     virtual bool hasLocalChanges();    
};
