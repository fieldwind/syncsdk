...
// These are not syncML commands. 
// These are actions (http upload or download) used in media sync.
 ...
 */
class SyncSourceReport : public ArrayElement {
...
private:
    ...
    /// The error type (optional) of the last error occurred for this source.
    ...
    std::map<std::string,ItemReport*>* clientAddItems;
    std::map<std::string,ItemReport*>* clientModItems;
    std::map<std::string,ItemReport*>* clientDelItems;
    std::map<std::string,ItemReport*>* clientDownloadedItems;

    // List of ItemReports for server modifications
    std::map<std::string,ItemReport*>* serverAddItems;
    std::map<std::string,ItemReport*>* serverModItems;
    std::map<std::string,ItemReport*>* serverDelItems;
    std::map<std::string,ItemReport*>* serverUploadedItems;

    // Return true if status is [200 <-> 499] (successful)
    // exception: code 420 = 'device full' is a failure status.
    ...
    void deleteItems(std::map<std::string,ItemReport*>* itemsMap);
    
    void cloneMap(std::map<std::string,ItemReport*>* srcMap,std::map<std::string,ItemReport*>* tgtMap);
...
public:
    ...
    /// (optional) the type of the last error code
    const char* getLastErrorType() const;

    /** the current state of the source, see SourceState */
    SourceState   getState() const;

    /// the current state of the source, as an explanation string.
    ...
    void setLastErrorType(const char* type);
    ...
    /**
     * Returns the total number of items succesfully synced, both Server and Client side.
     */
    int getTotalSuccessfulCount();
    ...
    ItemReport* find(const char* target, const char* command, const WCHAR* ID);
     ...
     * @param target     to select if client/server side          (values = CLIENT - SERVER)
     * @param command    to select the desired list of ItemReport (values = Add - Replace - Delete)
     * @return           a pointer to the desired ArrayList and its corrisponded set for fast searches
     *
     */
    std::map<std::string,ItemReport*>* getMap(const char* target, const char* command) const;
    ...
    /// Clones and returns a new allocated SyncSourceReport
    ArrayElement* clone();
};
