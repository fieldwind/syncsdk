...
class SyncManager {
    ...
    private:
        ...
        class IncomingSyncItem : public SyncItem {
		  private:
			IncomingSyncItem(const WCHAR* key) : SyncItem(key) {
			}
          public:
            ...
            StringBuffer cmdName; // name of the command which started the incomplete item
            StringBuffer cmdRef;  // reference of the command which started the incomplete item
            int sourceIndex;      // the index of the source to which the incomplete item belongs
            StringBuffer name;
            StringBuffer guid;
        ...
        void applySourceChanges(ArrayList &statusList);
