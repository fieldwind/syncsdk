...
// Error types. More to add.
#define ERROR_TYPE_MH_SYNC_MANAGER          "MHM-"
#define ERROR_TYPE_MH_REQUEST_MANAGER       "MHR-"
#define ERROR_TYPE_SAPI_SYNC_MANAGER        "SSM-"
#define ERROR_TYPE_SAPI_REQUEST_MANAGER     "SRM-"
#define ERROR_TYPE_WINDOWS_CLIENT           "WIN-"
#define ERROR_TYPE_IPHONE_CLIENT            "IPH-"
...
private:
    ...
    /// The error type (optional) of the last error occurred.
    StringBuffer lastErrorType;

    // Array of SyncSourceReport for each SyncSource.
    ArrayList ssReport;
...
public:
    ...
    int getSyncSourceReportCount() const;
    ...
    SyncSourceReport* getSyncSourceReport(int index) const;


    /**
     * ---- DEPRECATED! ----
     * Create ssReport array from config.
     * The array is allocated new, will be freed in the destructor.
    ...
    /**
     * Adds a SyncSourceReport to the array of SyncSourceReports.
     * The passed SyncSourceReport is cloned, so it can be then safely
     * deleted by the caller.
     * @return the number of SyncsourceReports in the array, after the addition
     *         a value < 0 in case of error
     */
    int addSyncSourceReport(SyncSourceReport& sourceReport);
