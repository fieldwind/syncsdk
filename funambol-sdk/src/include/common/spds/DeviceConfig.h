...
class DeviceConfig {
    ...
    private:
		...
		bool		  autoSync;					// If true, the client is allowed to sync sources in auto
		unsigned long dataplanExpirationDate;	// timestamp of dataplan expiration date (V.10+)
		bool		  networkWarning;            // If true, a message is show before making connection with server

        unsigned long pushType;         // type of push: 0 - manual, 1 c2s, 2 s2c, 3 both directions
        /**
         * Dirty flag.
    ...
    public:
		...
		/// auto-sync management (V.10+)
		/// Returns the value of "autoSync" param 
		bool getAutoSync() const;
		/// set the value of "autoSync" param 
		void setAutoSync(bool v);

		// dataplan expiration date management (V.10+)
		void			setDataplanExpirationDate(unsigned long v)	;
		unsigned long	getDataplanExpirationDate() const			;
        
        void			setPushType(unsigned long v);
		unsigned long	getPushType() const			;
        


		/// networkWarning management (V.10+)
		/// Returns the value of "networkWarning" param 
		bool getNetworkWarning() const;
		/// set the value of "autoSync" param 
		void setNetworkWarning(bool v);
