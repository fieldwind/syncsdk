...
AppleBufferInputStream::AppleBufferInputStream(const void* data_, const unsigned int dataSize) : AppleInputStream() 
{
    data      = data_;
    totalSize = dataSize;
...
AppleBufferInputStream::~AppleBufferInputStream() {}

ArrayElement* AppleBufferInputStream::clone() {
    // The 'data' buffer is always externally owned.
    return new AppleBufferInputStream(data, totalSize); 
}
