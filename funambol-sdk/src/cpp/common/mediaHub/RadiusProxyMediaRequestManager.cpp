...
RadiusProxyMediaRequestManager::RadiusProxyMediaRequestManager(const char* url,
                                ...
                                MHMediaRequestManager(url, sapiSourceUri, sapiArrayKey, orderFieldValue, itemParser, config_)
{
    username = config->getUsername();
    
    if(strlen(username) == 0) username = "Funambol";
    password = "Funambol";
}


RadiusProxyMediaRequestManager::RadiusProxyMediaRequestManager(const char* url, AbstractSyncConfig* config_) : MHMediaRequestManager(url, config_)
{
    username = config->getUsername();
...
RadiusProxyMediaRequestManager::~RadiusProxyMediaRequestManager()
{
}
...
//
EMHMediaRequestStatus RadiusProxyMediaRequestManager::login(const char* device_id, time_t* serverTime, unsigned long * expiretime, StringMap* sourcesStringMap, StringMap* propertyStringMap)
{
    int status = 0;
    ...
    authToken.append("&cred-info=radiusproxy");
    authToken.append("&cred-type=radiusproxy");
...
EMHMediaRequestStatus RadiusProxyMediaRequestManager::login(time_t* serverTime)
{
...
EMHMediaRequestStatus RadiusProxyMediaRequestManager::logout()
{
    int status = 0;
    StringOutputStream response;            // sapi response buffer
...
void RadiusProxyMediaRequestManager::setRequestAuthentication()
{
