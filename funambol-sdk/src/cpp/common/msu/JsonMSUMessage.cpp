...
static std::vector<std::string> getParameters(cJSON* root);
...
const char* JsonMSUMessage::formatSignUp(const MSUDeviceInfo *deviceInfo, bool prettyPrint)
{
    ...
    if ((language = deviceInfo->getL10N())) {
        cJSON_AddStringToObject(user, "l10n", language);
    }
...
const char* JsonMSUMessage::formatFieldsToCheckValidity(const char* username, const char* password, const char* language,
                                                        bool prettyPrint)
{
    char* formattedObject = NULL;
    cJSON *root = NULL, 
    ...
    *user  = NULL;
    ...
    if (username != NULL) {
        cJSON_AddStringToObject(user, "phonenumber", username);
    } else {
        cJSON_AddStringToObject(user, "phonenumber", "");
    }
    ...
    if (language != NULL) {
        ...
        cJSON_AddStringToObject(user, "l10n", "");
    }
...
std::vector<std::string> getParameters(cJSON* root) {
    std::vector<std::string> res;
    
    if (root != NULL) {
        cJSON* parameters = cJSON_GetObjectItem(root, "parameters");
        if (parameters != NULL) {
            int parametersSize = cJSON_GetArraySize(parameters);
            for(int i=0; i<parametersSize; i++) {
                cJSON* param = cJSON_GetArrayItem(parameters, i);
                cJSON* p = cJSON_GetObjectItem(param, "param");
                if (p != NULL && p->valuestring != NULL) {
                    res.push_back(p->valuestring);
                }
            }
        }
    }
