...
EMSUStatusCode MSUManager::login(const char* username, const char* password)
{
    ...
    if ((status = httpConnection->request(NULL, response)) != HTTP_OK) {
        ...
        switch (status) {
            ...
            case HTTP_FORBIDDEN:
                return EMSUForbidden;
                
            case HTTP_PAYMENT_REQUIRED:
                return EMSUPaymentRequired;
...
EMSUStatusCode MSUManager::signup(const char* username, const char* password,
                                  const char* phoneNumber,
                                  const char *token,
                                  MSUDeviceInfo* deviceInfo, std::string& serverErrorMsg)
{
    // safe checks
    if (((username == NULL) || (password == NULL)) ||
        ((strlen(username) == 0) || (strlen(password) == 0))) {
    ...
    if ((status = jsonMessage->parseSignUp(sapiResponse, serverErrorMsg)) == false) 
    {
        ...
        if (!strcmp(errorCode, sapiErrorInvalidCaptcha)) {                  // Invalid captcha: PRO-1126
            ...
            // Grab the server error message if present
...
EMSUStatusCode MSUManager::signupNoCaptcha(const char* username, const char* password, 
...
EMSUStatusCode MSUManager::checkFieldsValidity(const char* username, const char* password, const char* language,
    ...
    // Format the signup request body
    //
    const char* requestBody = jsonMessage->formatFieldsToCheckValidity(username, password, language);
    if (requestBody == NULL) {
        LOG.error("%s: error formatting signup request", __FUNCTION__);
        return EMSUSapiMessageFormatError;
    }
    ...
    if ((status = jsonMessage->parseFieldsToCheckValidity(sapiResponse, customErrMsg)) == false)
    {
        const char* errorCode = jsonMessage->getErrorCode().c_str();
        const char* errorMsg  = jsonMessage->getErrorMessage().c_str();
        if (!errorCode || !errorMsg) {
