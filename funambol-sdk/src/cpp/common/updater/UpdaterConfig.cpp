...
StringBuffer UpdaterConfig::updateTypePropertyName("update-type");
StringBuffer UpdaterConfig::nextRemindCheckPropertyName("next-remind-check");
...
bool UpdaterConfig::read()
{
    ...
    ConfigurationNode* mainNode = mainTree.getNode(CONTEXT_UPDATER);
    if (mainNode == NULL) {
    ...
    tmp = mainNode->readPropertyValue(updateTypePropertyName.c_str());
    LOG.debug("update-type=%s", tmp);
    updateType = tmp;
    delete [] tmp;
    
    tmp = mainNode->readPropertyValue(nextRemindCheckPropertyName.c_str());
    LOG.debug("next-remind-check=%s", tmp);
    nextRemindCheck = tmp;
    delete [] tmp;
...
void UpdaterConfig::save()
{
    ...
    s.sprintf("%ld", (long int)intervalCheckUpdateHttp);
    mainNode->setPropertyValue(intervalCheckUpdateHttpPropertyName.c_str(), s.c_str());

    s.sprintf("%ld", (long int)intervalRemind);
    ...
    mainNode->setPropertyValue(updateTypePropertyName.c_str(), updateType.c_str());    
    mainNode->setPropertyValue(nextRemindCheckPropertyName.c_str(), nextRemindCheck.c_str());
...
const StringBuffer& UpdaterConfig::getUpdateType() const {
    return updateType;
}

void  UpdaterConfig::setUpdateType(const StringBuffer& type) {
...
const StringBuffer& UpdaterConfig::getReleaseDate() const {
...
void UpdaterConfig::setNextRemindCheck(const StringBuffer& time) {
    nextRemindCheck = time;
}

const StringBuffer& UpdaterConfig::getNextRemindCheck() const {
    return nextRemindCheck;
}