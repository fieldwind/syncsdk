...
void SyncItemListener::itemUpdatedByClient(SyncItemEvent& event) {
    logEvent("item updated by client", event);
}

// listen for the Item uploaded by Client Event
void SyncItemListener::itemUploadedByClient(SyncItemEvent& event) {
    logEvent("item uploaded by client", event);
}


// Media sync listeners
void SyncItemListener::itemUploading(SyncItemEvent& event) {
    logEvent("start uploading", event);
}
void SyncItemListener::itemUploaded(SyncItemEvent& event) {
    logEvent("upload completed", event);
}
void SyncItemListener::itemDownloading(SyncItemEvent& event) {
    logEvent("start downloading", event);
}
void SyncItemListener::itemDownloaded(SyncItemEvent& event) {
    logEvent("download complete", event);
}
