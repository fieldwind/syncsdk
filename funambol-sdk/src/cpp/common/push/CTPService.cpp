 ...
 */
CTPService* CTPService::getInstance(DMTClientConfig* clientConfig_) {

    if (pinstance == NULL) {
        pinstance = new CTPService(clientConfig_);
    }
    return pinstance;
}
 ...
 */
CTPService::CTPService(DMTClientConfig* clientConfig_) : 
    config(APPLICATION_URI), clientConfig(clientConfig_) {
    ...
    pthread_mutex_init(&ctpSocketMtx, NULL);
}
...
CTPService::~CTPService() {
    ...
    pthread_mutex_destroy(&ctpSocketMtx);
}
...
int32_t CTPService::closeConnection() {
    ...
    pthread_mutex_lock(&ctpSocketMtx);
    ...
    pthread_mutex_unlock(&ctpSocketMtx);
...
int32_t CTPService::sendAuthMsg(){
    ...
    CTPParam username;
    StringBuffer configUserName = clientConfig->getUsername();
    username.setParamCode(P_USERNAME);
    username.setValue(configUserName.c_str(), configUserName.length());
...
int32_t CTPService::sendMsg(CTPMessage* message) {
    ...
    if (ret != msgLength) {
        ...
        cmdTimeoutThread = threadPool.createCmdTimeoutThread(this);
...
int32_t CTPService::receive() {
    ...
    //
    heartbeatThread = threadPool.createHeartbeatThread(this);
    heartbeatThread->start();
    
    //
    // Start thread to receive messages from Server
    //
    receiverThread = threadPool.createReceiverThread(this);
...
StringBuffer CTPService::createMD5Credentials() {
    ...
    const char*  username    = clientConfig->getUsername(); 
    const char*  password    = clientConfig->getPassword();
...
// CmdTimeoutThread
//////////////////////////////////////////////////////////////////////////////
CmdTimeoutThread::CmdTimeoutThread(CTPService* ctpService_) : ctpService(ctpService_) {
}
...
// CTPThread
//////////////////////////////////////////////////////////////////////////////
CTPThread::CTPThread(CTPService* ctpService_) :  FThread(),
                         errorCode(0), ctpService(ctpService_)
{
}
...
// ReceiverThread
//////////////////////////////////////////////////////////////////////////////
ReceiverThread::ReceiverThread(CTPService* ctpService_) : FThread(),
                                   errorCode(0), ctpService(ctpService_) 
{
}
 ...
 * @return 0 if no errors
 */
HeartbeatThread::HeartbeatThread(CTPService* ctpService_) : FThread(), ctpService(ctpService_)
{
}
