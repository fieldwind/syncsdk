...
#define PROPERTY_CRED_ENCRYPTED         "dataEncrypted"
...
void CTPConfig::readCTPConfig() {
    ...
    if (node) 
    {
        ...
        tmp = node->readPropertyValue(PROPERTY_CRED_ENCRYPTED);
        if (tmp) {
            setDataEncrypted(atoi(tmp));
        } else {
            setDataEncrypted(0);
        }                
        delete [] tmp;
...
//    node = dmt->readManagementNode(nodeName.c_str());
//    if (node) {
//        saveAuthConfig(*node, *node); // first node not used
//        delete node;
//    }
    ...
    if (node) 
    {
        ...
        value.sprintf("%d", dataEncrypted);
        node->setPropertyValue(PROPERTY_CRED_ENCRYPTED, value.c_str());
        
        node->setPropertyValue(CTP_NONCE, ctpNonce.c_str());
