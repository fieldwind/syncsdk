...
AccessConfig::AccessConfig() {
    ...
    responseTimeout       = 0;
    compression           = false;
    encryptionMode        = NOT_ENCRYPTED;
    oauth2AccessToken     = "";
    oauth2AccessTokenSetTime = 0;
    oauth2RefreshToken    = "";
    oauth2ExpiresIn       = "";
    oauth2Scope           = "";
    oauth2ClientType      = "";
...
AccessConfig::~AccessConfig() {
    ...
    safeDelete(&phoneidentify           );
    safeDelete(&tokenauth           );
...
const char* AccessConfig::getPhoneNumber() const {
    return phonenum;
}

void AccessConfig::setPhoneNumber(const char* v) {
    set(&phonenum, v);
    
    dirty |= DIRTY_PHONENUM;
}
...
const char*  AccessConfig::getPhoneidentify() const{
    return phoneidentify;
}
void AccessConfig::setPhoneidentify(const char*  v){
    set(&phoneidentify, v);
}

const char*  AccessConfig::getTokenauth() const{
    return tokenauth;
}

void AccessConfig::setTokenauth(const char*  v){
    set(&tokenauth, v);
}
...
AccessConfig::EncryptionMode AccessConfig::getEncryptionMode() const {
    return encryptionMode;
}

void AccessConfig::setEncryptionMode(AccessConfig::EncryptionMode mode) {
    encryptionMode = mode;
}
...
void AccessConfig::assign(const AccessConfig& s) {
    setAuthenticationType(s.getAuthenticationType());
    ...
    setPhoneidentify(s.getPhoneidentify());
    setTokenauth(s.getTokenauth());
    ...
    setEncryptionMode(s.getEncryptionMode());

    setOAuth2AccessToken(s.getOAuth2AccessToken());
    setOAuth2AccessTokenSetTime(s.getOAuth2AccessTokenSetTime());
    setOAuth2RefreshToken(s.getOAuth2RefreshToken());
    setOAuth2ExpiresIn(s.getOAuth2ExpiresIn());
    setOAuth2Scope(s.getOAuth2Scope());
    setOAuth2ClientType(s.getOAuth2ClientType());
    
    setWassupAuthToken(s.getWassupAuthToken());
