...
static bool isItemTooBig(EncodingHelper& helper     ,
    ...
    int percMaxMsgSize = (int)(0.05*maxMsgSize);
    if (maxMsgSize < changeOverhead + msgSize) {
        ...
        return (itemSize > (maxMsgSize - changeOverhead - msgSize));
    }
    return false;
}
...
bool SyncManager::testIfDataSizeMismatch(long allocatedSize, long receivedSize) {
    ...
    if (declaredSize != receivedSize) {
        LOG.debug("Warning: item size mismatch: real size = %d, declared size = %d", receivedSize, declaredSize);
        return true;
    }
    return false;
}
...
void SyncManager::initialize() {
    ...
    credentialHandler.setOAuth2AccessTokenSetTime(config.getOAuth2AccessTokenSetTime());
    credentialHandler.setOAuth2Credentials  (config.getOAuth2AccessToken(), config.getOAuth2RefreshToken(),
                                             config.getOAuth2ClientType(), false /* not actually used btw */,
                                             atoi(config.getOAuth2ExpiresIn()));
...
int SyncManager::prepareSync(SyncSource** s) {
    ...
    StringBuffer oldOAuth2AccessToken("");
    ...
    LOG.debug(MSG_SYNC_URL, syncURL.c_str());
    ...
    //config.setBeginSync(timestamp); not used actually
    ...
    do {
        ...
        if (isClientAuthenticated == false) {
            ...
            if (cred && cred->getAuthentication() && cred->getAuthentication()->getPassword()) {
                strcpy(credentialInfo, cred->getAuthentication()->getPassword());
            }
        }
        
        // to check if it's changed after server response
        oldOAuth2AccessToken = credentialHandler.getOAuth2Credentials().getAccessToken();
        ...
        for (count = 0; count < sourcesNumber; count ++) {
            ...
            // Alers Status 402: payment required. v.10+
            if (ret == -1 || ret == 404 || ret == 415 || ret == 402 ) {
        ...
        for (int i=0; i < list->size(); i++) {
            ...
            if ( (responseCmd = syncMLProcessor.processGetCommand(cmd, devInf, syncMLBuilder)) ) {
        ...
        for (int i=0; i < list->size(); i++) {
            AbstractCommand* cmd = (AbstractCommand*)list->get(i);
            ArrayList* responseCmd = NULL;
            if ( (responseCmd = syncMLProcessor.processPutCommand(cmd, config, syncMLBuilder)) ) {
                commands.add(responseCmd);
                delete responseCmd;
            }
        }
        ...
        if (isAuthFailed(ret)) {
            ...
            if (strcmp(credentialHandler.getClientAuthType(),requestedAuthType) != 0 ) {
                if (clientChal && clientChal->getNextNonce()) {
                    if (strcmp(requestedAuthType, AUTH_TYPE_MD5) == 0) {
                        credentialHandler.setClientNonce(clientChal->getNextNonce()->getValueAsBase64());
                    } else if (strcmp(clientChal->getType(), AUTH_TYPE_OAUTH) == 0) {
                        credentialHandler.setOAuth2Credentials(clientChal->getNextNonce()->getValueAsBase64());
                    } else if (strcmp(clientChal->getType(), AUTH_TYPE_RADIUS_PROXY) == 0) {
                        credentialHandler.setClientNonce(clientChal->getNextNonce()->getValueAsBase64());
                    }
                }
            } else {
                if (clientChal && clientChal->getNextNonce()) {
                    if (strcmp(requestedAuthType, AUTH_TYPE_MD5) == 0 && clientAuthRetries == 1)  {
                        credentialHandler.setClientNonce(clientChal->getNextNonce()->getValueAsBase64());
                    } else if (strcmp(requestedAuthType, AUTH_TYPE_OAUTH) == 0 && clientAuthRetries == 1)  {
                        credentialHandler.setOAuth2Credentials(clientChal->getNextNonce()->getValueAsBase64());
                    } else if (strcmp(clientChal->getType(), AUTH_TYPE_RADIUS_PROXY) == 0) {
                        credentialHandler.setClientNonce(clientChal->getNextNonce()->getValueAsBase64());
                    } else {
            ...
            // auth OK (212)
            if (clientChal && clientChal->getNextNonce()) {
                if (strcmp(clientChal->getType(), AUTH_TYPE_MD5) == 0) {
                    credentialHandler.setClientNonce(clientChal->getNextNonce()->getValueAsBase64());
                }
                else if (strcmp(clientChal->getType(), AUTH_TYPE_OAUTH) == 0) {
                    credentialHandler.setOAuth2Credentials(clientChal->getNextNonce()->getValueAsBase64());
    ...
    if (!oldOAuth2AccessToken.empty() &&
        oldOAuth2AccessToken != credentialHandler.getOAuth2Credentials().getAccessToken()) {
        // oauth2 token has changed (server returned a new one): set all oauth params to config
        config.setOauth2Params(credentialHandler.getOAuth2Credentials(), time(NULL));
    }
...
void SyncManager::applySourceChanges(ArrayList &statusList) {
    sources[count]->applyItems(items);
    ArrayList previousStatus;
    for (int i = 0; i < items.size(); i++) {
        IncomingSyncItem* item = (IncomingSyncItem*)items[i];
        StringBuffer command;
        Status* status = NULL;
        int code = item->getSyncStatus();
        if (item->getState() == SYNC_STATE_NEW) {
            status = syncMLBuilder.prepareItemStatus(ADD, item->name, item->cmdRef, code);
            ...
            if ((code >= 200 && code <= 299) || code == STC_ALREADY_EXISTS) {     // send mapping for code 418 too
                char *key = toMultibyte(item->getKey());
                mmanager[item->sourceIndex]->addMapping(key, item->guid); // LUID, GUID
                delete [] key;
            }
        }
        else if (item->getState() == SYNC_STATE_UPDATED) {
            status = syncMLBuilder.prepareItemStatus(REPLACE, item->name, item->cmdRef, code);
            command = COMMAND_REPLACE;
        }
        else if (item->getState() == SYNC_STATE_DELETED) {
            status = syncMLBuilder.prepareItemStatus(DEL, item->name, item->cmdRef, code);
            command = COMMAND_DELETE;
        }
        if (!command.empty()) {
            // Fire Sync Status Event: item status from client
            fireSyncStatusEvent(status->getCmd(), status->getStatusCode(), sources[item->sourceIndex]->getConfig().getName(), sources[item->sourceIndex]->getConfig().getURI(), item->getKey(), CLIENT_STATUS);
            // Update SyncReport
            sources[item->sourceIndex]->getReport()->addItem(CLIENT, command.c_str(), item->getKey(), status->getStatusCode(), NULL);
        }
        if (status) {
            syncMLBuilder.addItemStatus(&previousStatus, status);
            deleteStatus(&status);
    ...
    for (count = 0; count < sourcesNumber; count ++) {
        ...
        if ( sources[count]->beginSync() ) {
            ...
            if (getLastErrorCode() == 0) {
                setError(ERR_UNSPECIFIED, "Error in begin sync");
            }
            ret = getLastErrorCode();
        ...
        do {
                    ...
                    do {
                        ...
                        if (syncItem) {
                            ...
                            syncMLBuilder.addChunk(modificationCommand,
                                                   ADD_COMMAND_NAME, /*modify by zhaojunjie @20160314*/
                                                   syncItem,
                                                   chunk,
                            ...
                            // add the key item in the list of mod items.
                            syncItemKeys.insertModKey(StringBuffer().convert(syncItem->getKey()));
                ...
                default: // TWO-WAY sync
                {
                    ...
                    if (step == 1) {
                        do {
                            ...
                            // add the key item in the list of add items.
                            syncItemKeys.insertAddKey(StringBuffer().convert(syncItem->getKey()));
                    ...
                    if (step == 5) {
                        do {
                            ...
                            // add the key item in the list of mod items.
                            syncItemKeys.insertDelKey(StringBuffer().convert(syncItem->getKey()));
            ...
            //
            syncMLBuilder.setTarget(transportAgent->getURL().fullURL); //add by zhaojunjie
            ...
            if (responseMsg == NULL || !responseMsg[0]) {
                ret=getLastErrorCode();
                if (ret == 0) {
                    setError(ERR_REPRESENTATION, "Response message is NULL");
            ...
            // source method
            //
            int itemret = syncMLProcessor.processItemStatus(*sources[count], syncml->getSyncBody(), syncItemKeys);
            if(itemret){
                char *name = toMultibyte(sources[count]->getName());
...
int SyncManager::endSync() {
    ...
    for (count = 0; ret == 0 && count < sourcesNumber; count ++) {
        ...
        LOG.debug("Committing changes for source '%s'", sources[count]->getConfig().getName());
        
        commitChanges(*sources[count]);
    }
    
    //config.setEndSync((unsigned long)time(NULL)); not used actually
    safeDelete(&responseMsg);
    safeDelete(&mapMsg);
    if (list){
        delete list;
...
bool SyncManager::commitChanges(SyncSource& source) {
    ...
    if (ssconfig) {
        ...
        ssconfig->setEndSyncTime((unsigned long)time(NULL));
        return true;
    } else {
        return false;
    }
...
Status *SyncManager::processSyncItem(Item* item, const CommandInfo &cmdInfo, SyncMLBuilder &syncMLBuilder)
{
    ...
    if (incomingItem) {
        ...
        incomingItem = new IncomingSyncItem(iname, cmdInfo, count, itemName, NULL == item->getSource() ? NULL : item->getSource()->getLocURI());
    ...
    if (incomingItem) {
        ...
        const char* source_uri  = sources[count]->getConfig().getURI();
        const char* source_name = sources[count]->getConfig().getName();
            ...
            if (incomingItem) {
                ...
                if (incomingItem->offset == 0) {
                    ...
                    if (!strcmp(cmdInfo.commandName, REPLACE)) {
                        ...
                        eventType = ITEM_DELETED_BY_SERVER; // actually never used...
                    }
                    fireSyncItemEvent(source_uri, source_name, incomingItem->getKey(), eventType);
                }
                
                incomingItem->offset += size;
            }
        } else if (!strcmp(cmdInfo.commandName, DEL)) {
            fireSyncItemEvent(source_uri, source_name, incomingItem->getKey(), ITEM_DELETED_BY_SERVER);
        }
...
void SyncManager::initTransportAgent(const URL& url) {
    ...
    StringBuffer phoneidentify(config.getPhoneidentify());
    transportAgent->setPhoneidentify(phoneidentify);
    
    StringBuffer tokenaut(config.getTokenauth());
    transportAgent->setTokenauth(tokenaut);

    
    // Update the user agent string in the transport agent, making sure it is
    // valid.  If the config property 'user agent' is empty, it is replaced by
    // 'mod' and 'SwV' properties from AbstractDeviceConfig.
    // If also 'mod' property is empty, sets a default user agent.
    ...
    if (userAgent.empty()) {
        ...
        if (mod && strcmp(mod, "")) {
            userAgent.append(mod);
            if (swV && strcmp(swV, "")) {
                userAgent.append(" ");
                userAgent.append(swV);
            }
        } else {
    ...
    LOG.debug("User Agent = %s", userAgent.c_str());
    transportAgent->setUserAgent(userAgent);
    
    transportAgent->setCompression(config.getCompression());
    
    // Set a default content type
    transportAgent->setProperty(TA_PropertyContentType, SYNCML_CONTENT_TYPE);
    
    // Set the deviceId for each http request
    transportAgent->setProperty(TA_PropertyDeviceId, deviceId);
}
...
ArrayElement* SyncManager::IncomingSyncItem::clone() {
    IncomingSyncItem* ret = new IncomingSyncItem(key);
    ret->offset = offset;
    ret->cmdName = cmdName;
    ret->cmdRef = cmdRef;
    ret->sourceIndex = sourceIndex;
    ret->name = name;
    ret->guid = guid;
