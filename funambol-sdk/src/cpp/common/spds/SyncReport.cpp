        ...
        if (verbose) {
            for (const char* const* target = SyncSourceReport::targets;
                ...
                for (const char* const* commands = SyncSourceReport::commands;
                     *commands;
                     commands++) {
                    const std::map<std::string,ItemReport*>* itemsMap = ssr->getMap(*target, *commands);
                    if (itemsMap) {
                        std::map<std::string,ItemReport*>::const_iterator itemsIt = itemsMap->begin();
                        for(;itemsIt != itemsMap->end();++itemsIt) {
                            std::pair<std::string,ItemReport*> itemPair = *itemsIt;
                            ItemReport* item = itemPair.second;
