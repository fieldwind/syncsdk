...
#define VER_1_1         "1.1"
#define VER_1_2         "1.2"
#define PROTOCOL_DM	    "DM"
#define PROTOCOL_SYNCML "SyncML"
#define PROTOCOL_VER(flavour, version) flavour "/" version

void CreateProtocolInfo(SyncMLProtocolFlavour protocol, VerDTD *&pVerDTD, VerProto *&pVerProto) {
    const char *verNo = VER_1_2, *verProto = PROTOCOL_VER(PROTOCOL_SYNCML, VER_1_2);
    switch (protocol) {
        case SYNCML_DM_1_1:
            verNo = VER_1_1; verProto = PROTOCOL_VER(PROTOCOL_DM, VER_1_1); break;
        case SYNCML_DM_1_2:
            verNo = VER_1_2; verProto = PROTOCOL_VER(PROTOCOL_DM, VER_1_2); break;
        case SYNCML_1_0:
            verNo = VER_1_0; verProto = PROTOCOL_VER(PROTOCOL_SYNCML, VER_1_0); break;
        case SYNCML_1_1:
            verNo = VER_1_1; verProto = PROTOCOL_VER(PROTOCOL_SYNCML, VER_1_1); break;
        case SYNCML_1_2:
            verNo = VER_1_2; verProto = PROTOCOL_VER(PROTOCOL_SYNCML, VER_1_2); break;
    }
    
    pVerDTD   = new VerDTD(verNo);
    pVerProto = new VerProto(verProto);
}


SyncMLBuilder::SyncMLBuilder(SyncMLProtocolFlavour p) : protocol(p) {
    initialize();
    set(NULL, NULL);
}
...
SyncMLBuilder::SyncMLBuilder(char* t, char* d, SyncMLProtocolFlavour p) :
protocol(p)
{
    initialize();
    set(t, d);
...
void SyncMLBuilder::setTarget(const char*t) {
...
SyncHdr* SyncMLBuilder::prepareSyncHdr(Cred* cred, unsigned long maxMsgSize, unsigned long maxObjSize) {
    ...
    CreateProtocolInfo(protocol, verDTD, verProto);
    ...
    if (cred /*&& strcmp(cred->getType(), AUTH_TYPE_MD5) == 0*/)
    {
        ...
        username = stringdup(cred->getUsername());
    }
    else
        sou = new Source(device);
    
    sou = new Source(device, username);
...
Item* SyncMLBuilder::prepareItemChunk(SyncItem* syncItem,
    ...
    if (strcmp(DELETE_COMMAND_NAME, COMMAND) != 0) {
        ...
        const char* tp = toMultibyte(syncItem->getDataType());
        if (tp) {
            m.setType(tp);
        }
...
long SyncMLBuilder::addChunk(ModificationCommand* &modificationCommand,
                             const char* COMMAND, SyncItem* syncItem, Chunk* chunk,
    ...
    if (!type || !type[0]) {
        // TODO: use stringbuffer
        WCHAR* t = toWideChar(defaultType);
        syncItem->setDataType(t);
        delete [] t;
