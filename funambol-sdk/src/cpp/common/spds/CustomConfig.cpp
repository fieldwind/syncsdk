...
void CustomConfig::setLongLongProperty(const char* propertyName, long long propertyValue) {
    StringBuffer s;
    s.sprintf("%lld", propertyValue);
    extraProps.put(propertyName, s.c_str());
}

long long CustomConfig::getLongLongProperty(const char* propertyName, bool *err) {
    const char* ret = extraProps.get(propertyName);
    long long result = -1;
