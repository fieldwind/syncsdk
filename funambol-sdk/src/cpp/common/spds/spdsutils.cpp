...
const SyncMode getSyncCapsMode(const int type) {
    for (int i=0; syncCapsMapping[i].type >= 0; i++) {
        if (syncCapsMapping[i].type == type) {
            return syncCapsMapping[i].mode;
        }
    }
    // if not found
    return SYNC_NONE;
...
const int getSyncCapsType(const SyncMode mode) {
    for (int i=0; syncCapsMapping[i].type >= 0; i++) {
        if (syncCapsMapping[i].mode == mode) {
            return syncCapsMapping[i].type;
        }
    }
    // if not found
    return -1;
...
ArrayList* syncModesStringToList(const StringBuffer& syncModesString) {
    ...
    ArrayList* syncModeList = new ArrayList();
    char buffer[80];
    const char *mode = syncModesString.c_str();
    ...
    while (*mode) {
        ...
        SyncType syncType(getSyncCapsType(sm));
        if (syncType.getType() != -1) {
...
StringBuffer syncModesListToString(const ArrayList& syncModesList) {
    ...
    StringBuffer syncModesString;

    for (int i=0; i<syncModesList.size(); i++) {
        SyncType* syncType = (SyncType*)syncModesList.get(i);
        int type = syncType->getType();             // i.e: type = 1
        SyncMode mode = getSyncCapsMode(type);      // i.e: mode = 200
        StringBuffer name(syncModeKeyword(mode));   // i.e: name = "two-way"

        // append if valid
        if (!name.empty() && name != "none") {
            if (!syncModesString.empty()) {
                syncModesString.append(",");
            }
            syncModesString.append(name);
        }
    }
    return syncModesString;
}
