...
#include "ioStream/BoundaryInputStream.h"
...
int64_t BoundaryInputStream::read(void* buffer, const int64_t size) {
...
int BoundaryInputStream::setPosition(int64_t offset)
{
    if (offset <= totalSize) {
        eofbit = 0;
        position = offset;
...
int64_t BoundaryInputStream::getPosition() {
    return position;
}
