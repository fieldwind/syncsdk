...
#include "ioStream/MultipleInputStream.h"
...
int64_t MultipleInputStream::read(void* buffer, const int64_t size) {
    ...
    if (!stream) {
        LOG.error("MultipleInputStream: stream #%i not available", currentSection+1);
        return 0;
    }

    // Reads data from the current stream
    int64_t bytesRead = readFromStream(stream, buffer, size);
    position += bytesRead;
...
int64_t MultipleInputStream::readFromStream(InputStream* stream, void* buffer, const int64_t size) {
...
int64_t MultipleInputStream::getPosition() {
    return position;
}

int MultipleInputStream::setPosition(const int64_t offset) {
    LOG.error("*** MultipleInputStream::setPosition not implemented ***");
    return 1;
}
