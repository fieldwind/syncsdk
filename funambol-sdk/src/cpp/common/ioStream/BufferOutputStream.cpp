...
int64_t BufferOutputStream::writeBuffer(const void* buffer, int64_t size) {
    ...
    if (bytesWritten == 0) {
        // first time
        dataAllocSize = std::max(size, (int64_t)MIN_ALLOC_SIZE);
    ...
    if (bytesWritten + size > dataAllocSize) {
        ...
        dataAllocSize = bytesWritten + std::max(size, (int64_t)MIN_ALLOC_SIZE);
