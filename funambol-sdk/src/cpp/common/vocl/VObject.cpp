...
void VObject::removeProperty(const WCHAR* propName) {
...
VProperty* VObject::getProperty(const WCHAR* propName) {
	/*
    VProperty* property = (VProperty*)properties->front();
    while (NULL != property && wcscmp(property->getName(), propName)) {
        property = (VProperty*)properties->next();
    }
    return property;
	*/
    for (int i=0; i<properties->size(); i++) {
...
WCHAR* VObject::getPropertyValue(const WCHAR *propertyName) {
    WCHAR* propertyValue = NULL;
    VProperty* vprop = getProperty(propertyName);
...
void VObject::removeAllProperies(const WCHAR* propName) {
...
VProperty* VObject::getFirstProperty() {
	return (VProperty*)properties->front();
}

VProperty* VObject::getNextProperty() {
	return (VProperty*)properties->next();
}
