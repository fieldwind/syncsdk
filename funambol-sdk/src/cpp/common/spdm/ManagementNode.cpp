 ...
 *
 */
ManagementNode::ManagementNode(const char* parent, const char* name) : ConfigurationNode(parent, name) {
}

ManagementNode::ManagementNode(const char* fullname) : ConfigurationNode(NULL,NULL) {
 ...
 */
void ManagementNode::addChild(ConfigurationNode* node) {
	children.add(*node);
}

void ManagementNode::removeAllChildren() {
    children.clear();
}

StringMap* ManagementNode::readAllProperties() { 
    StringMap* ret = new StringMap();
    ret->put(PROPERTY_DUMMY_KEY, "");
    return ret;
}

int ManagementNode::deleteProperty(const char* keyName) {
    LOG.info("%s default deletePropertyNode called. NOT IMPLEMENTED.");
    return 0;
}
