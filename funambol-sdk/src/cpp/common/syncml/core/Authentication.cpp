...
void Authentication::createAuthentication(const char* type, const char* data) {
    ...
    else if (strcmp(realtype, AUTH_TYPE_OAUTH) == 0) {
        this->setType(AUTH_TYPE_OAUTH);
        this->setData(data);
    }
    else if (strcmp(realtype, AUTH_TYPE_RADIUS_PROXY) == 0) {
        this->setType(AUTH_TYPE_RADIUS_PROXY);
        this->setFormat(FORMAT_B64);
        this->setData(data);
    }
}
...
void Authentication::setData(const char*data) {
    ...
    if (strcmp(type,AUTH_TYPE_BASIC) == 0 || strcmp(type,AUTH_TYPE_RADIUS_PROXY) == 0) {
        char* clearData = NULL;
    ...
    else if (strcmp(type, AUTH_TYPE_OAUTH) == 0) {
        if (meta->getFormat() == NULL) {
            this->setFormat(FORMAT_B64);
        }
        this->data = stringdup(data);
