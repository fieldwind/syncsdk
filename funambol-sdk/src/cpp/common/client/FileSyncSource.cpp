...
static StringBuffer getRelativeName(const StringBuffer& dir, const StringBuffer& fullName) {
...
Enumeration* FileSyncSource::getAllItemList() {
    ...
    LOG.debug("The total number of files found is %i", filesFound.size());
...
int FileSyncSource::insertItem(SyncItem& item) {
    ...
    if (file.parse(item.getData(), item.getDataSize()) == 0) {
        // Not all the items are accepted: some may be filtered out
        if (filterIncomingItem(file) == false) {
...
StringBuffer FileSyncSource::formatMetadata(const WCHAR* key) {
    
    StringBuffer ret;
    if (!key) {
        LOG.error("formatMetadata error: NULL key");
        return ret;
    }
    
    StringBuffer completeName = getCompleteName(dir, key);
    LOG.debug("complete name = %s", completeName.c_str());
    
    // Opens the file.
    // NOTE: MUST open in binary mode, in oder to have a correct position indicator
    // of the stream after each read() call.
    FILE* f = fileOpen(completeName.c_str(), "rb");
    if (!f) {
        // File not existing (OK: can happen if it's a Delete). Return empty string.
        return "";
    }
    ...
    // Set a NULL body: to avoid formatting a "body" tag.
    fileData.setBody(NULL, 0);

    // Sets the file modification time (UTC string like "20091028T192200Z")
    unsigned long tstamp = getFileModTime(completeName.c_str());
    ...
    if (!buf) {
        LOG.error("formatMetadata error: cannot format file data object");
        return ret;
    }
...
bool FileSyncSource::filterIncomingItem(FileData& file) {
