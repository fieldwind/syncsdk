...
#include "client/MediaSyncSourceSyncMLData.h"
...
MediaSyncSourceSyncMLData::MediaSyncSourceSyncMLData(const WCHAR* wname, 
    ...
    LOG.debug("MediaSyncSourceSyncMLData: config params file is %s", configParamsName.c_str());
    ...
    if (params.getFilterBySize()) {
        LOG.debug("MediaSyncSourceSyncMLData: the size filtering is ON (max file size = %u KB)", params.getFilterBySize());
    } else {
        LOG.debug("MediaSyncSourceSyncMLData: the size filtering is OFF");
    }
    if (params.getFilterByDate()) {
        filterDateString = unixTimeToString(params.getFilterByDate(), true);
        LOG.debug("MediaSyncSourceSyncMLData: the date filtering is ON");
        LOG.info("Files modified before %s will not be sent", filterDateString.c_str());
    } else {
        filterDateString = "";
        LOG.debug("MediaSyncSourceSyncMLData: the date filtering is OFF");
    }
    ...
    LOG.debug("MediaSyncSourceSyncMLData: LUID map file is %s", mapFileName.c_str());
...
MediaSyncSourceSyncMLData::~MediaSyncSourceSyncMLData() 
{
    if (LUIDMap) { 
        delete LUIDMap;
    }
...
int MediaSyncSourceSyncMLData::beginSync() {
...
bool MediaSyncSourceSyncMLData::checkCacheValidity()
{
...
int MediaSyncSourceSyncMLData::saveCache()
{
...
int MediaSyncSourceSyncMLData::insertItem(SyncItem& item) 
{
    // Items are not added from Server to Client.
    // So just log a warning and return OK.
    StringBuffer key;
    key.convert(item.getKey());
    LOG.debug("Warning: unexpected call MediaSyncSourceSyncMLData::insertItem() for item key = %s", key.c_str());
    
    return STC_COMMAND_NOT_ALLOWED;
}

int MediaSyncSourceSyncMLData::modifyItem(SyncItem& item) 
{
    // Items are not modified from Server to Client.
    // So just log a warning and return OK.
    StringBuffer key;
    key.convert(item.getKey());
    LOG.debug("Warning: unexpected call MediaSyncSourceSyncMLData::modifyItem() for item key = %s", key.c_str());
    
    return STC_COMMAND_NOT_ALLOWED;
}

int MediaSyncSourceSyncMLData::removeItem(SyncItem& item) 
{
    // Items are not deleted from Server to Client.
    // So just log a warning and return OK.
    StringBuffer key;
    key.convert(item.getKey());
    LOG.debug("Warning: unexpected call MediaSyncSourceSyncMLData::removeItem() for item key = %s", key.c_str());
    
    return STC_COMMAND_NOT_ALLOWED;
}


bool MediaSyncSourceSyncMLData::filterOutgoingItem(const StringBuffer& fullName, struct stat& st)
{
...
StringBuffer MediaSyncSourceSyncMLData::getItemSignature(StringBuffer& key) {
    ...
    LOG.debug("[%s] MediaSyncSourceSyncMLData - getting signature for item with key %s", getConfig().getName(), key.c_str());       
    StringBuffer s;
...
SyncItem* MediaSyncSourceSyncMLData::getFirstItem() {
...
SyncItem* MediaSyncSourceSyncMLData::getFirstNewItem() {    
...
void MediaSyncSourceSyncMLData::dynamicFilterItems(Enumeration* itemKeys) {
...
bool MediaSyncSourceSyncMLData::dynamicFilterItem(const StringBuffer& fileName) {
    ...
    if (filterDate) {
        LOG.debug("MediaSyncSourceSyncMLData: filtering date = %s", filterDateString.c_str());
...
void MediaSyncSourceSyncMLData::fireClientTotalNumber(int number) {
    // void implementation (to avoid firing event from CacheSyncSource)
}

SyncItem* MediaSyncSourceSyncMLData::fillSyncItem(StringBuffer* key, const bool fillData)
{
    ...
    if (syncItem && key) {
        ...
        LOG.debug("MediaSyncSourceSyncMLData::fillSyncItem - LUID of item '%s' is %s", key->c_str(), luid.c_str());
...
void MediaSyncSourceSyncMLData::getKeyAndSignature(SyncItem& item, KeyValuePair& kvp)
{
    // FIX the item's key.
...
void MediaSyncSourceSyncMLData::setItemStatus(const WCHAR* wkey, int status, const char* command) {
    ...
    if (!isErrorCode(status)) {
        ...
        if (status == STC_DEVICE_FULL) {
            // status 420: Server quota exceeded
            // Set the error, to avoid sending other items (see fillItems() method)
            setSourceError(ERR_SERVER_QUOTA_EXCEEDED);
            setError(ERR_SERVER_QUOTA_EXCEEDED, report->getLastErrorMsg());

            LOG.error("%s", report->getLastErrorMsg());
        }
        // it's an error status: doesn't update the cache
    }
...
void MediaSyncSourceSyncMLData::setSourceError(const int errorCode) {
...
StringBuffer MediaSyncSourceSyncMLData::getLUIDFromPath(const StringBuffer& path)
{
...
StringBuffer MediaSyncSourceSyncMLData::getPathFromLUID(const StringBuffer& luid)
{
    ...
    if (path.null()) {
        ...
        LOG.error("MediaSyncSourceSyncMLData - path not found in LUIDMap for LUID '%s'!", luid.c_str());
    }
    return path;
}
...
bool MediaSyncSourceSyncMLData::verifyNextLUIDValue()
{
    bool updated = false;
    Enumeration& props = LUIDMap->getProperties();
...
bool MediaSyncSourceSyncMLData::refreshLUIDMap()
{
    bool updated = false;
    Enumeration& props = LUIDMap->getProperties();
...
const int MediaSyncSourceSyncMLData::readNextLUID()
{
    int ret = 0;
...
void MediaSyncSourceSyncMLData::saveNextLUID(const int nextLUID)
{
    StringBuffer value;
