...
bool OptionParser::parse(int argc, const char** argv, StringMap &opts, ArrayList &args) {
    ...
    for (int i=1; i<argc; i++) {
        ...
        if (arg[0] == '-') {
            ...
            if (opt->hasArgument) { // check if a value has been provided to option
            ...
            } else {  // doesn't have required argument but might be have somehting
                ...
                if (argv[++i] && (strlen(argv[i]) > 0)) {
                    if (argv[i][0] == '-') {	
                        ...
                        opts.put(opt->longName, argv[i]);   // something has been found		
                    }
				} else {
                    opts.put(opt->longName, "1");  // nothing found
                }
            }
