...
int SyncClient::sync(AbstractSyncConfig& config, SyncSource** sources) {
    ...
    // Set the last sync results in config
    setLastSyncResults(config);
...
void SyncClient::setLastSyncResults(AbstractSyncConfig& config) {
    ...
    // Set the global sync result
    config.setLastGlobalError(getLastErrorCode());

    // Set the sync results for all sources synced
    int count = syncReport.getSyncSourceReportCount();
    for (int i=0; i < count; i++) {
        
        SyncSourceReport* ssr = syncReport.getSyncSourceReport(i);
        if (ssr && (ssr->getState() != SOURCE_INACTIVE)) {

            StringBuffer sourceName = ssr->getSourceName();
            AbstractSyncSourceConfig* ssc = config.getAbstractSyncSourceConfig(sourceName.c_str());
            ssc->setLastSourceError(ssr->getLastErrorCode());
        }
    }
}
