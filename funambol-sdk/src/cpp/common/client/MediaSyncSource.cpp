...
void MediaSyncSource::setItemStatus(const WCHAR* wkey, int status, const char* command) {
    ...
    if (!isErrorCode(status)) {
        ...
        LOG.error("[%s], Received failed status code from server for %s on item with key %s - code: %d", 
                  getConfig().getName(), command, key.c_str(), status);
...
bool MediaSyncSource::isErrorCode(int code) {
    ...
    else if (code == STC_ALREADY_EXISTS) {
        return false;
    }
    else {
        return true;
