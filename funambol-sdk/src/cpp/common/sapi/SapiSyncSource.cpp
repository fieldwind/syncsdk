...
SapiSyncSource::SapiSyncSource(SyncSourceConfig& sc, SyncSourceReport& rep, size_t incomingFilterD, size_t outgoingFilterD, const char* storageLocation, bool recursive_) 
                : config(sc), report(rep), incomingFilterDate(incomingFilterD), outgoingFilterDate(outgoingFilterD), recursive(recursive_) {
...
SapiSyncSource::SapiSyncSource(SyncSourceConfig& sc, SyncSourceReport& rep, KeyValueStore* mappings, 
               KeyValueStore* cache, KeyValueStore* blacklist, size_t incomingFilterD, size_t outgoingFilterD, const char* storageLocation, bool recursive_) 
                : config(sc), report(rep),  incomingFilterDate(incomingFilterD), outgoingFilterDate(outgoingFilterD), recursive(recursive_)  {
...
void SapiSyncSource::sortByModificationTime(ArrayListEnumeration* list, int maxItemsToSort) {
    ...
    if (!list) { 
        LOG.debug("%s sorting list is empty", __FUNCTION__);
        return; 
    }
...
bool SapiSyncSource::filterFromBlackList(SapiSyncItemInfo& itemInfo) {
    ...
    return filterFromBlackList(guid);
}

bool SapiSyncSource::filterFromBlackList(StringBuffer& guid) {
...
bool SapiSyncSource::filterOutgoingItem(SapiSyncItemInfo& clientItemInfo) {
    ...
    //
    // 1. Check for item in blacklist
    //
    if (filterFromBlackList(clientItemInfo.getLuid())) {
        return true;
    }

    //
    // 2. Check for item size  (0 means disabled)
    //
    ...
    if (!err && maxItemSize>0) {
        ...
        if (itemSize > (unsigned int)maxItemSize) {
            StringBuffer sb = clientItemInfo.getName();
            LOG.info("%s: discarding local item %s of size %lu (maximum item size allowed exceeded)",
                __FUNCTION__, sb.c_str(), itemSize);
            return true;
        }
    }
    
    //
    // 3. Filter by the number of item to send out. If there is a number in the config it uses it 
    ...
    //
    // 4. Check if the modification date is over the filter date
    //
    if (outgoingFilterDate > 0) {
        if (clientItemInfo.getModificationDate() >= outgoingFilterDate) {
            return false;  // we keep it
        } else {
            StringBuffer modDate   = unixTimeToString((unsigned long)clientItemInfo.getModificationDate(), false);
...
bool SapiSyncSource::checkModifiedLocalItems(AbstractSyncConfig& mainConfig) {
        ...
        while(allItemInfo->hasMoreElement()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (strcmp(kvp->getKey(), luid.c_str()) == 0) {
                    ...
                    if (itemInfo->getModificationDate() != val) {
                        // 1 updated item found!
                        if (filterOutgoingItem(*itemInfo) || 
                            filterByExtension(*itemInfo)) {
                            LOG.debug("%s: found an updated %s item (%s) but filtered so no update raised", 
                                    __FUNCTION__, config.getName(), itemInfo->getName().c_str());
                            cacheCopy.removeElementAt(i);
                            break;
                        }
                        LOG.debug("%s: found an updated %s item (%s), exiting", __FUNCTION__, config.getName(), itemInfo->getName().c_str());
                        return true;
                    }
                    cacheCopy.removeElementAt(i);
                    break;
            ...
            if (foundnew) {
                // 1 new item found: check the outgoing filters + extensions
                if (filterOutgoingItem(*itemInfo) || 
                    filterByExtension(*itemInfo)) {
                    continue;
                }
                LOG.debug("%s: found a new %s item (%s), exiting", __FUNCTION__, config.getName(), itemInfo->getName().c_str());
                return true;
            }
        }
    }
    ...
    if (cacheCopy.size() > 0) {
        // these are deleted items!
        LOG.debug("%s: found one or more deleted %s items, exiting", __FUNCTION__, config.getName());
        return true;
    }
...
bool SapiSyncSource::filterIncomingItem(SapiSyncItemInfo& serverItemInfo, time_t offsetTime) {
    ...
    if (incomingFilterDate > 0) {
        if (serverItemInfo.getModificationDate() < (incomingFilterDate - offsetTime)) {
            ...
            StringBuffer modDate  = unixTimeToString((unsigned long)serverItemInfo.getModificationDate(), false);
...
UploadSapiSyncItem* SapiSyncSource::getNextModItem(int* err) {
    ...
    // v10 LOG.info("[%s] - UPDATE FROM CLIENT NOT YET IMPLEMENTED", __FUNCTION__);
    
    // v11: send update as well
...
SapiSyncItemInfo* SapiSyncSource::getNextDelItem(int* err) {
    ...
    // v10 LOG.debug("[%s] - DELETE FROM CLIENT NOT YET IMPLEMENTED", __FUNCTION__);
...
void SapiSyncSource::setItemStatus(SapiSyncItemInfo& itemInfo, int status, const char* command) {
    ...
    if (!isErrorCode(status)) {
        ...
        if (strcmp(command, COMMAND_DELETE)) {
            ...
            LOG.error("Received failed status code from server for command %s on source %s on item with key %s - code: %d", 
                            command, config.getName(), itemInfo.getLuid().c_str(), status);                    
    }    
    if (vp.getKey()) {
...
StringBuffer SapiSyncSource::updateItem(DownloadSapiSyncItem* syncItem, ESapiSyncSourceError* errCode) {    
    ...
    if (syncItem->getSapiSyncItemInfo()->getLuid() != res) {
        kp.setKey(syncItem->getSapiSyncItemInfo()->getLuid());
...
int SapiSyncSource::endUpload() {
    int errCode = report.getLastErrorCode();

    if ((errCode == 0) || (errCode == ESSSItemNotSupported)) {
        config.setIntProperty(PROPERTY_SYNC_ITEM_NUMBER_FROM_CLIENT, -1);
    }
...
int SapiSyncSource::endDownload() {
    ...
    if ((errCode == 0) || (errCode == ESSSItemNotSupported)) {
        config.setIntProperty(PROPERTY_SYNC_ITEM_NUMBER_FROM_SERVER, -1);
    }
    ...
    for (int i=0; i<newItemInfo->size(); i++) {
        ...
        for (int j=0; j<deletedItemInfo->size(); j++) {
            ...
            if (newItem->getModificationDate() == delItem->getModificationDate()) {
                ...
                // NOT DONE: v11 Moved into SapiSyncManager - rename handling
