...
bool SapiSyncManager::checkLocalChanges() {

    LOG.debug("%s: source %s", __FUNCTION__, sourceName.c_str());
    ...
    // define the sync behaviour (ALL items, changed items) with these 2 calls
    isSyncingItemChanges = defineSyncBehavior();
    checkLastUserAndServer();
    
    // ALL items (1st sync): 
    // return true if at least 1 local item is found
    if (isSyncingItemChanges == false) {
        // other safe check to avoid mega loop. If the error code is != 0
        // and in this condition, it might happen an error in the first sync
        // with item exchanged. so continue without first sync check
        if (source.getConfig().getLastSourceError() != 0) {
            LOG.debug("%s: source %s: found source error != 0 as first sync. Check as usual sync", 
                __FUNCTION__, sourceName.c_str());
        } else {
        return source.checkAllLocalItems(config);
        }
    }

    // Item changes (next sync): 
    // return true if at least 1 NEW/MOD/DEL item is found
    SyncMode syncMode = syncModeCode(source.getConfig().getSync());
    if (isUploadEnabled(syncMode)) {
        if (source.populateAllItemInfoList(config) == false) {
            return false;
        }
        return source.checkModifiedLocalItems(config); 
    }
    return false;
}
        ...
        for (int i=0; i<allClientItems->size(); i++) {
            ...
            if (findLuidInMappings(itemInfo->getLuid().c_str())) {
                ...
                if (clientFilterNumber > 0) {
                    ...
                    LOG.debug("clientFilterNumber decreased: %d", clientFilterNumber);
                }
                continue;
            }
        }
    ...
    for (int i=0; i<allServerItems.size(); i++) {
        ...
        if (findGuidInMappings(itemInfo->getGuid().c_str())) {
            ...
            if (serverFilterNumber > 0) {
                ...
                LOG.debug("serverFilterNumber decreased: %d", serverFilterNumber);
            }
            continue;
        }
...
int SapiSyncManager::upload() 
{
    ...
    if (isSyncingItemChanges) {
        ...
        {
            deleteItem(*clientItemInfo);
            delete clientItemInfo;
        }
    }
    else {
        //
...
int SapiSyncManager::resumeUploads() {
    ...
    while (resumeIDs.hasMoreElement()) 
    {
        ...
        if (type == RESUME_UPLOAD) {
            ...
            // One item less to upload!
            if (clientFilterNumber > 0) {
                clientFilterNumber --;
                LOG.debug("clientFilterNumber decreased for resume: %d", clientFilterNumber);
            }
        }
        delete oldItemInfo;
...
int SapiSyncManager::resumeUpload(UploadSapiSyncItem& resumeItem, SapiSyncItemInfo& oldItemInfo, const StringBuffer& command) 
{
    ...
    if (itemInfo->getSize() != oldItemInfo.getSize()) {
        ...
        offset = 0;
        LOG.debug("Getting resume informations on item to upload: %s", itemInfo->getName().c_str());
        ...
        if (err != ESMRSuccess) {
            if (err == ESMRInvalidParam || err == ESMRSapiMessageFormatError || 
                err == ESMRHTTPFunctionalityNotSupported || err == ESMRSapiNotSupported || 
                err == ESMRInvalidContentRange  || err == ESMRUnknownMediaException) {
                // For these error codes, we DON'T resume operation but retry item upload from scratch  
                
                LOG.info("Item '%s' can't be resumed: will be uploaded from scratch", itemInfo->getName().c_str());
                removeFromResumeMap(*itemInfo);
                resumeItem.getSapiSyncItemInfo()->setGuid("");
                uploadStatus = uploadItem(resumeItem, COMMAND_ADD, false);
                goto finally;
            }
...
int SapiSyncManager::deleteItem(SapiSyncItemInfo& itemInfo) {

    StringBuffer command = COMMAND_DELETE;
    ...
    if (luid.empty()) {
        LOG.error("Internal error: empty delete item LUID");
        setSyncError(ESSMGenericSyncError);
        return report.getLastErrorCode();
    }
    ...
    if (guid.empty()) {
        LOG.error("Internal error: empty delete item GUID");
        setSyncError(ESSMGenericSyncError);
        return report.getLastErrorCode();
    }
    ...
    // delete item
    LOG.debug("Deleting item: %s", itemInfo.getName().c_str());
    err = sapiMediaRequestManager->deleteItem(&itemInfo);
    ...
    if (err == ESMRAccessDenied) {
        ...
        err = sapiMediaRequestManager->login(config.getDevID());
        if (err == ESMRSuccess) {
            LOG.debug("%s: retrying delete metadata for item: %s", __FUNCTION__, itemInfo.getName().c_str()); 
    ...
    source.getMappings().removeProperty(guid.c_str());

    // delete entry in the ServerDate mappings
    source.getServerDateMappings().removeProperty(guid.c_str());
   
    // fire syncItem event
    fireSyncItemEvent(sourceURI.c_str(), sourceName.c_str(), wluid.c_str(), ITEM_UPLOADED);
    LOG.debug("Delete completed (guid = %s)", guid.c_str());
    deleteStatus = 0;   // success

finally:
    if (deleteStatus != 0) {
        deleteStatus = report.getLastErrorCode();
    }

    // always update report, even if error!
    report.addItem(SERVER, command.c_str(), wluid.c_str(), deleteStatus, NULL);
...
int SapiSyncManager::uploadItem(UploadSapiSyncItem& clientItem, const StringBuffer& command, bool isResume) {
    ...
    if (command == COMMAND_REPLACE && itemInfo->isRename() && !isResume) {
        ...
        guid = itemInfo->getGuid();
        if (guid.empty()) {
            LOG.error("%s: empty item GUID, cannot proceed with rename", __FUNCTION__);
            setSyncError(ESSMGenericSyncError);
            goto finally;
        }

        LOG.debug("Rename for item: %s", itemInfo->getName().c_str());
        ESapiMediaRequestStatus err = sapiMediaRequestManager->renameItem(itemInfo, &lastUpdate);
        ...
        if (err == ESMRAccessDenied) {
            ...
            if (err == ESMRSuccess) {
                ...
                // retry with new session
                err = sapiMediaRequestManager->renameItem(itemInfo, &lastUpdate);
            }
        }
        if (err != ESMRSuccess) {
            setSyncError(ESSMSapiError, err);
    ...
    if (err != ESMRSuccess) {
        ...
        if (err == ESMRQuotaExceeded          || err == ESMRInvalidParam ||
            ...
            if (err == ESMRUnknownMediaException || err == ESMRInvalidItemPermanentlyRefused) {
                source.getBlacklist().setPropertyValue(itemInfo->getLuid(), itemInfo->getGuid());    
                LOG.info("Item '%s' could not be uploaded: item blacklisted so it won't be uploaded again", itemInfo->getLuid().c_str());
                // treat it as a particular error
                err = ESMRInvalidItemPermanentlyRefused;
                setSyncError(ESSMInvalidItemPermanentlyRefused, err);                                
                goto finally;
            }
        }
        setSyncError(ESSMSapiError, err);
...
int SapiSyncManager::downloadItem(SapiSyncItemInfo& itemInfo, const StringBuffer& command, bool isResume) {
    ...
    if (command == COMMAND_REPLACE && itemInfo.isRename() && !isResume) {
        ...
        // create fake serverItem with NULL output stream
        SapiSyncItemInfo* info = (SapiSyncItemInfo*)itemInfo.clone();
        serverItem = new DownloadSapiSyncItem(info, NULL);        
...
SapiMediaSourceType SapiSyncManager::getMediaSourceType() {
    ...
    else if (sourceName == "music")    return ESapiMediaSourceMusic;
...
int SapiSyncManager::readClientFilterNumber() {
    ...
    if (num > 0) {
        LOG.info("Filter by number is set: max %d items will be uploaded", num);
    }
    return num;
}
...
int SapiSyncManager::readServerFilterNumber() {
    ...
    if (num > 0) {
        LOG.info("Filter by number is set: max %d items will be downloaded", num);
    }
    return num;
}
...
bool SapiSyncManager::shouldStopAllDownloads(const int errorCode) {

    // for these codes, we continue with next item
    if ((errorCode != ESSMSuccess) && 
        (errorCode != ESSMSapiError) &&
        (errorCode != ESSMSetItemError) && 
        (errorCode != ESSMItemNotSupportedBySource)) {
        return true;
    }

    // for all other codes, we stop the downloads
    return false;
}

bool SapiSyncManager::shouldStopAllUploads(const int errorCode) {

    // for these codes, we continue with next item
    if ((errorCode != ESSMSuccess) && 
        (errorCode != ESSMSapiError) && 
        (errorCode != ESSMInvalidItemPermanentlyRefused)) {
        return true;
    }

    // for all other codes, we stop the uploads
    return false;
}
        ...
        case ESSMSapiError:
        {
            ...
            switch (sapiStatus)
            {
                case ESMRAccessDenied:
                {
                    ...
                    msg.sprintf("SAPI authentication error, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                    break;
                }
                ...
                case ESMRSapiNotSupported:
                {
                    ...
                    msg.sprintf("SAPI request not implemented by server, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                ...
                case ESMRRequestTimeout:
                {
                    ...
                    msg.sprintf("Network error, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                    break;
                }
                ...
                {   
                    msg.sprintf("HTTP error, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                    break;
                }
                
                case ESMRInvalidContentRange:
                {
                    msg.sprintf("HTTP range header error, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                    break;
                }
                ...
                default:
                {
                    msg.sprintf("SAPI request error, caused by code %s%d", ERROR_TYPE_SAPI_REQUEST_MANAGER, data);
                    break;
                }
            }
            break;
...
bool SapiSyncManager::areThereModifications(bool add, bool mod, bool del, const char* clientOrServer) {
    
    
    if (clientOrServer == NULL || (clientOrServer && strcmp(clientOrServer, "CLIENT") == 0)) {
        if (add && source.getItemsNumber("NEW") > 0) { return true; }
        if (mod && source.getItemsNumber("MOD") > 0) { return true; }
        if (del && source.getItemsNumber("DEL") > 0) { return true; }     
        if (add && totalClientItems > 0)             { return true; }     
    } 
    if (clientOrServer == NULL || (clientOrServer && strcmp(clientOrServer, "SERVER") == 0)) {
        if (add && newServerItems.size() > 0) { return true; }
        if (mod && modServerItems.size() > 0) { return true; }
        if (del && delServerItems.size() > 0) { return true; }   
        if (add && totalServerItems > 0)      { return true; } 
    }
    return false;
