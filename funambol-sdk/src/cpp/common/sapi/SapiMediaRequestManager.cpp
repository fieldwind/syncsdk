...
static const char* getUriWithIdsFmt   = "%s/sapi/media/%s?action=get&id=%s&responsetime=true&exif=none&shared=true";
...
static const char* sapiGetInfoUrlFmt  = "%s/sapi/system/information?action=get";
...
static const char* renameItemData      = "%s/sapi/media/%s?action=update&lastupdate=true";
...
static const char* sapiMusicUrlToken = "audio";
...
static const char* sapiJsonArrayMusicKey = "audios";
...
static const char* sapiInvalidItemPermanentlyRefused  = "COM-1100";
    ...
    username(user_name), password(pass)
{
    ...
    switch (sourceType) {
        ...
        case ESapiMediaSourceFiles:
            ...
            break;
        case ESapiMediaSourceMusic:
            sapiMediaSourceName = sapiMusicUrlToken;
            break;
...
ESapiMediaRequestStatus SapiMediaRequestManager::getAllItems(ArrayList& sapiItemInfoList, time_t* responseTime, int limit, int offset)
{
    ...
    switch (sourceType) {
        ...
        case ESapiMediaSourceFiles:
            ...
            break;
        case ESapiMediaSourceMusic:
            itemsArrayKey = sapiJsonArrayMusicKey;
            break;
        case ESapiMediaSourceContacts:
                                                                 ...
                                                                 time_t* reponseTimestamp)
{
    ...
    switch (sourceType) {
        ...
        case ESapiMediaSourceFiles:
            ...
            break;
        case ESapiMediaSourceMusic:
            sourceTokenName = sapiMusicUrlToken;
            break;
        case ESapiMediaSourceContacts:
...
ESapiMediaRequestStatus SapiMediaRequestManager::uploadItemMetaData(UploadSapiSyncItem* item)
{
    ...
    if (jsonSapiMediaObjectParser->parseMediaAddItem(itemMetaDataUploadJson, itemId, &lastUpdate, &parserStatus) == false) {
        ...
        if (!strcmp(errorCode, sapiSecurityException)) {               
            ...
            return ESMRSapiNotSupported;
        } else if (!strcmp(errorCode, sapiInvalidItemPermanentlyRefused)) {
            return ESMRInvalidItemPermanentlyRefused;
        }
...
ESapiMediaRequestStatus SapiMediaRequestManager::uploadItemData(UploadSapiSyncItem* item, time_t* lastUpdate)
{
    ...
    if ((status != HTTP_OK) && (status != HTTP_PARTIAL_CONTENT)) {
        LOG.error("%s: error %d sending upload request", __FUNCTION__, status);
        httpConnection->close();

        fireTransportEvent(-1, SEND_DATA_END);
    ...
    const char* responseString = response.getString().c_str();
    ...
    LOG.debug("%s: sapi item add result = %s", __FUNCTION__, responseString);
    
    if (jsonSapiMediaObjectParser->parseMediaAddItem(responseString, itemGuid, lastUpdate, &parserStatus) == false) {
        const char* errorCode = jsonSapiMediaObjectParser->getErrorCode().c_str();
        const char* errorMsg  = jsonSapiMediaObjectParser->getErrorMessage().c_str();
...
ESapiMediaRequestStatus SapiMediaRequestManager::getItemResumeInfo(UploadSapiSyncItem* item, size_t* offset)
{
    ...
    *offset = 0;
    ...
    if (status == HTTP_OK) {
        ...
        if (jsonSapiMediaObjectParser->checkErrorMessages(response.getString().c_str()) == false) {
            
            const char* errorCode = jsonSapiMediaObjectParser->getErrorCode().c_str();
            const char* errorMsg  = jsonSapiMediaObjectParser->getErrorMessage().c_str();
            LOG.error("%s: SAPI error %s: %s", __FUNCTION__, errorCode, errorMsg);
            
            if (!strcmp(errorCode, sapiUnknownMediaException) || !strcmp(errorCode, sapiErrorRetrievingMediaItem)) {
        ...
        if (rangeValues.size() != 2) {
            ...
            if ((*offset = atol(rangeOffset->c_str())) <= 0) {
                LOG.error("%s: error parsing HTTP range headers: invalid range offeset [%d]", 
                        __FUNCTION__, *offset);
                httpConnection->close();
...
ESapiMediaRequestStatus SapiMediaRequestManager::getQuotaInfo(unsigned long long* free, unsigned long long* quota)
{
    ...
    quotaInfoUrl.sprintf(getQuotaInfoUrlFmt, serverUrl.c_str());
...
ESapiMediaRequestStatus SapiMediaRequestManager::login(const char* device_id, time_t* serverTime, unsigned long * expiretime, StringMap* sourcesStringMap, StringMap* propertyStringMap)
{
    ...
    sapiLoginUrl.sprintf(sapiLoginUrlFmt, serverUrl.c_str(), deviceIdEncoded);
	...
	LOG.debug("Performing login with url: %s", sapiLoginUrl.c_str());
	...
	// post credentials into the body
    ...
    StringBuffer authToken("login=");
	authToken.append(usernameEncoded);
	authToken.append("&password=");
	authToken.append(passwordEncoded);
...
ESapiMediaRequestStatus SapiMediaRequestManager::getServerInformation(StringMap* responseParams)
{
    int status = 0;
    StringOutputStream response;            // sapi response buffer
    const char* sapiResponse = NULL;
    ESMPStatus parserStatus;
   
    // compose the request URL
    StringBuffer sapiUrl;
    sapiUrl.sprintf(sapiGetInfoUrlFmt, serverUrl.c_str());
    URL requestUrl = sapiUrl.c_str();
    ...
    if ((sapiResponse = response.getString().c_str()) == NULL) {
        LOG.error("%s: NULL sapi response to 'get Server information'", __FUNCTION__);
        return ESMRSapiInvalidResponse;
    }
    
    LOG.debug("%s: response returned = %s", __FUNCTION__, sapiResponse);


    if ((status = jsonSapiMediaObjectParser->parseServerInfo(sapiResponse, responseParams, &parserStatus)) == false) {
        LOG.error("%s: error parsing sapi login response", __FUNCTION__);
...
ESapiMediaRequestStatus SapiMediaRequestManager::deleteItem(SapiSyncItemInfo* itemInfo)
{
    int status = 0;
    const char* itemId = NULL;
    StringBuffer itemsListRequestUrl;
    ...
    if (jsonSapiMediaObjectParser->formatDelItemsListObject(itemsIDs, &itemsIdsListJsonObject, itemsArrayKey, true) == false) {
        LOG.error("%s: error formatting json object for items list", __FUNCTION__);
        
        return ESMRInvalidParam;
    }
   
    itemsListRequestUrl.sprintf(deleteItemData, serverUrl.c_str(), sapiMediaSourceName);
    ...
    if ((status = httpConnection->request(itemsIdsListJsonObject, response)) != HTTP_OK) {
        LOG.error("%s: error sending upload request", __FUNCTION__);
        httpConnection->close();
        free(itemsIdsListJsonObject);
    ...
    if (jsonSapiMediaObjectParser->checkErrorMessages(result) == false) {
        const char* errorCode = jsonSapiMediaObjectParser->getErrorCode().c_str();
        const char* errorMsg  = jsonSapiMediaObjectParser->getErrorMessage().c_str();
...
ESapiMediaRequestStatus SapiMediaRequestManager::renameItem(SapiSyncItemInfo* itemInfo, time_t* lastUpdate)
{
    int status = 0;
    const char* itemId = NULL;
    StringBuffer itemsListRequestUrl;
    ...
    if (jsonSapiMediaObjectParser->formatRenameItemsListObject(itemInfo, &itemsIdsListJsonObject, true) == false) {
        LOG.error("%s: error formatting json object for items list", __FUNCTION__);
        
        return ESMRInvalidParam;
    }
   
    itemsListRequestUrl.sprintf(renameItemData, serverUrl.c_str(), sapiMediaSourceName);
    ...
    LOG.debug("%s: sapi rename item metadata request response = %s", __FUNCTION__, result);
    
    
    if (jsonSapiMediaObjectParser->checkErrorMessages(result, lastUpdate) == false) {
        const char* errorCode = jsonSapiMediaObjectParser->getErrorCode().c_str();
        const char* errorMsg  = jsonSapiMediaObjectParser->getErrorMessage().c_str();
