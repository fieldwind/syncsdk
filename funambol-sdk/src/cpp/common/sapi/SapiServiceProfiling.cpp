...
SapiServiceProfiling::SapiServiceProfiling(AbstractSyncConfig& c) : config(c), 
                                                                    ...
                                                                    err(ESMRSuccess),
                                                                    autoSync(false),
                                                                    networkWarning(getNetworkWarning()) {
...
int SapiServiceProfiling::login() {
    ...
    if (requestDetails) {
        ...
        err = req.login(config.getDevID()); // added new parameters expireTime , sources, propertyStringMap...
    }
...
int SapiServiceProfiling::getSapiVersion() {

    StringMap responseParams;
    URL url(config.getSyncURL());
    StringBuffer host = url.getHostURL();
    ...
    ESMRStatus ret = req.getServerInformation(&responseParams);

    if (ret == ESMRSuccess) {
        const StringBuffer& version = responseParams.get("sapiversion");
        LOG.info("SAPI server info returned successfully, sapi version = \"%s\"", version.c_str());

        // set to sapi config
        config.setSapiVersion(version.c_str());
    }
