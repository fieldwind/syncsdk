...
bool SapiMediaJsonParser::parseLogin(const char* message, StringBuffer& sessionID, time_t* responseTime, ESMPStatus* errCode, unsigned long * expiretime, StringMap* sourcesStringMap, StringMap* propertyStringMap)
{
    cJSON *root = NULL;
    cJSON *data = NULL;
	...
	if (sourcesStringMap  != NULL && 
		...
		if ((details = cJSON_GetObjectItem(data, "details"))) {
...
bool SapiMediaJsonParser::parseServerInfo(const char* message, StringMap* responseParams, ESMPStatus* errCode)
{
    cJSON *root = NULL;
    cJSON *version = NULL;
    bool error = false;
    ...
    if (responseParams == NULL) {
        LOG.error("%s: invalid responseParams", __FUNCTION__);
        *errCode = ESMPInvalidArgument;
        return false;
    }
    ...
    // Get "sapiversion" field
    if ((version = cJSON_GetObjectItem(root, "sapiversion")) == NULL) {
        LOG.error("%s: missing sapiversion field in json object", __FUNCTION__);
        cJSON_Delete(root);
        *errCode = ESMPKeyMissing;
        return false;
    }

    if (version->valuestring == NULL) {
        LOG.error("%s: invalid sapi version in JSON object", __FUNCTION__);
        cJSON_Delete(root);
        *errCode = ESMPInvalidMessage;
        return false;
    }

    responseParams->put("sapiversion", version->valuestring);

    //
    // If needed, we may get all other server info fields here
    // (man, mod, utc, hwv, fwv, exts,...) and put them into responseParams map.
    //
...
bool SapiMediaJsonParser::parseRestoreChargeResponse(const char* message, ESMPStatus* errCode)
{
    cJSON *root = NULL;
    cJSON *data = NULL;
...
//
int SapiMediaJsonParser::jsonObjArray2StringMap(cJSON* objJsonArray, StringMap* stringMap) {
...
bool SapiMediaJsonParser::formatDelItemsListObject(const ArrayList& itemsIDs, char **itemsListJsonObject, 
...
bool SapiMediaJsonParser::formatRenameItemsListObject(SapiSyncItemInfo* itemInfo, char **itemsListJsonObject, bool prettyPrint)
{
    cJSON *root = NULL, *data = NULL;
    const char* itemGuid = NULL;
    const char* itemName = NULL;
    ...
    int itemId = atoi(itemGuid);
    cJSON_AddIntToObject(data, "id", (size_t)itemId);
...
bool SapiMediaJsonParser::checkErrorMessages(const char* response, time_t* lastUpdate)
{
    cJSON *root = NULL;
    cJSON *date = NULL;
    bool error = false;
