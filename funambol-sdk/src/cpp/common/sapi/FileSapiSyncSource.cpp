...
FileSapiSyncSource::FileSapiSyncSource(SyncSourceConfig& sc, SyncSourceReport& rep, size_t incomingFilterDate, size_t outgoingFilterDate, const char* storageLocation, bool recursive_) 
                                    : SapiSyncSource(sc, rep, incomingFilterDate, outgoingFilterDate, storageLocation, recursive_), 
                                      folderSize(0) {
}
...
bool FileSapiSyncSource::populateAllItemInfoList(AbstractSyncConfig& mainConfig) {
    ...
    bool ret = true;
    folderSize = 0;
    LOG.debug("Read all files from dir: %s [recursive: %d]", path.c_str(), recursive);
    ArrayList files = readFilesInDirRecursive(path.c_str(), recursive);
    int filesCount = files.size();  

    LOG.debug("Creating local allItemsInfo list [count %d]...", filesCount);
    ...
    for (int i=0; i<filesCount; i++) {
        ...
        if (file == NULL) {
            LOG.error("%s: can't access file at index %d of files list", __FUNCTION__, i);
            continue;
        }
        ...
        if (S_ISDIR(st.st_mode)) {
            LOG.info("[%s] file '%s' is a directory: skipping", __FUNCTION__, file->c_str());
            continue;   // skip subfolders: TODO fix here for recursive reading
        }

        // set size (it's mandatory)
        int fileSize = fgetsize(file->c_str());         
        ...
        if (pos != StringBuffer::npos) {            
            ...
            mime = SapiContentType::getContentTypeByExtension("");
        }
    ...
    LOG.debug("Total dir size = %d (dir: %s)", folderSize, path.c_str());

    return ret;
}

bool FileSapiSyncSource::checkAllLocalItems(AbstractSyncConfig& mainConfig) {
    ...
    //
    // Check if files are ok (extension filters, exclude folders, ., ..)
    //
    for (int i=0; i<filesCount; i++) {
        if (i%50 == 0) {
            if (mainConfig.isToAbort()) {
        ...
        // exclude items because of outgoing filters + extensions
        SapiSyncItemInfo info;
        info.setName(fileName.c_str());
        info.setSize(fileSize);
        info.setLuid(file->c_str());
        if (filterOutgoingItem(info) || filterByExtension(info)) {
            continue;
        }

        // if here, most likely this item is ok
        LOG.debug("%s: found a %s item (%s), exiting", __FUNCTION__, config.getName(), fileName.c_str());
        return true;
    }
...
StringBuffer FileSapiSyncSource::insertItem(DownloadSapiSyncItem* syncItem, ESapiSyncSourceError* errCode, long* modificationDate) {
    ...
    if (fileExists(realFileName.c_str())) {
        LOG.info("%s The file already exists (%s). Create a new one", __FUNCTION__, realFileName.c_str());
        for (int i = 0; i < 20; i++) {            
            StringBuffer tmp;
    ...
    if (renameFile(p.c_str(), realFileName.c_str()) != 0) {
        LOG.error("[%s] cannot rename the file %s. Deleting it", __FUNCTION__, p.c_str());
        if (removeFile(p.c_str()) == false) {
            LOG.error("[%s] cannot delete the file %s", __FUNCTION__, p.c_str());
        }
        *errCode = ESSSItemNotSupported; // avoiding mega loop. if cannot rename, delete tmp and blacklist
        // *errCode = ESSSInternalError;
        *modificationDate = 0;
        return "";
    }
...
StringBuffer FileSapiSyncSource::getFolderPath() {
    ...
    // If PROPERTY_MEDIAHUB_PATH is not absolute, prepend home dir
    if (*pp != '/') {
        path = PlatformAdapter::getHomeFolder() + "/" + pp;
    }
    else {
        path = pp;
    }
...
bool FileSapiSyncSource::isLocalStorageAvailable(int64_t size, int* errorCode) { 
...
void FileSapiSyncSource::validateLocalLists() {
    //
    // v11: Nothing to do: NEW/MOD/DEL items are supported, client to client.
    //
    //
    // v10: Only NEW and MOD items are supported, client to server.
    //
