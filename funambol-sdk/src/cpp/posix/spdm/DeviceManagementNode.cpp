...
void DeviceManagementNode::setCompatibilityMode(bool mode){
    ...
    if(mode){
        StringBuffer val(PlatformAdapter::getHomeFolder());
        val += "/.sync4j/";
        setConfigPath( val );
...
void DeviceManagementNode::lookupDir() {
    ...
    if (configPath.empty()){
        ...
        //StringBuffer configHome(PlatformAdapter::getConfigFolder());
        // This is the home og the user
        StringBuffer configHome;
        //if (configHome.empty()){
        StringBuffer userHome(PlatformAdapter::getHomeFolder());
#if not defined(FUN_IPHONE)
        configHome = userHome + "/.config";
#endif
          //  configHome.append("/");
          //  configHome.append(context);
            //configHome.append("/");
            //configHome.append(name);
        //}
...
void DeviceManagementNode::update(bool read) {
    ...
    if (gotoDir(read)) {
        ...
        StringBuffer tmpName(name);
        if ( tmpName.find(PROPERTY_DATASTORES, 0) != StringBuffer::npos){
            //do nothin
        }else {
...
int DeviceManagementNode::deletePropertyNode(const char* nodeName) {
    ...
    if (nodeName) {
        ...
        nodeNameToDelete = getConfigPath() + "/" + context + "/" + name + "/" + nodeName;
        //nodeNameToDelete = getConfigPath() + "/" + nodeName;
        cleanMultipleSlashes(nodeNameToDelete);
        if ( removeDir(nodeNameToDelete.c_str()) ) {
            err = 1;
        }
    }
    return err;
...
StringMap* DeviceManagementNode::readAllProperties() {
    ...
    while (true) {
        ...
        if (value) {
            ...
            tmp[1] = 0;
            smap->put(valcompare.c_str(), res);
        } else {
            smap->put(valcompare.c_str(), "");
        }       
        i++;
    }
