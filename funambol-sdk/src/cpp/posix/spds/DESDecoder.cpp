...
char* DESDecoder::transform(char* data, TransformationInfo& info)
{
    ...
    NSMutableData *decodedData = [NSMutableData dataWithLength:len + kCCBlockSizeDES];
    size_t outLength;
    ...
    CCCryptorStatus cryptStatus = CCCrypt(kCCDecrypt, kCCAlgorithmDES,
                                          ...
                                          vkey, kCCKeySizeDES,
                                          NULL, (void*)data, len,
                                          [decodedData mutableBytes], [decodedData length], &outLength);
    
    if (cryptStatus == kCCSuccess) {
        char* res = (char*)new char[outLength+1];
        memcpy(res, [decodedData bytes], outLength);
        res[outLength] = 0;
        info.size = outLength;
