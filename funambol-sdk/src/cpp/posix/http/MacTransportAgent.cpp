...
char * MacTransportAgent::sendBuffer(const void * data, const unsigned int size) 
{
    ...
    if (getTimeout() == 99) {
        LOG.debug("Request (logged in clear mode):\n%s", (const char*)data);
    } else {
        logMessage = removeSensibleData((const char*)data);        
        LOG.debug("Request:\n%s", logMessage.c_str());
    }
    ...
    if ((url.host == NULL) || (strlen(url.host) == 0)) {
        LOG.error("MacTransportAgent::sendMessage error: invalid server name");
        setError(ERR_NETWORK_INIT, "MacTransportAgent::sendMessage error: invalid server name");
        ...
        if (scnReachRef == NULL) {
            LOG.error("MacTransportAgent::sendMessage error: SCNetworkReachabilityCreateWithName error.");
            setError(ERR_NETWORK_INIT, "MacTransportAgent::sendMessage error: SCNetworkReachabilityCreateWithName error.");
    ...
    if ( gotflags && isReachable && noConnectionRequired ){
        ...
        setProperty(TA_Property_Authorization, getTokenauth());
        setProperty(TA_Property_X_3GPP_Intended_Identity, getPhoneidentify());
        ...
        handler = new HttpConnectionHandler(timeout, DEFAULT_INTERNET_READ_BUFFER_SIZE, NULL);
        for (numretries=0; numretries < MAX_RETRIES; numretries++) {
            ...
            if ((res = handler->startConnectionHandler(responseStream, size)) != 0) {
                if (res == ERR_CONNECT) {
                    ...
                    continue;
                } else if (res == ERR_CONNECTION_TIMEOUT) {
                    LOG.error("connection timeout");
                    statusCode = 408;
                } else if (res == E_REQUEST_CANCELLED) {
                    LOG.debug("%s: connection cancelled by user request", __FUNCTION__);
                    statusCode = -2;
                    goto exit;
                } else {
    ...
    exit:
        ...
        switch (statusCode) {
            ...
            case -2 : {
                        setErrorF(ERR_CONNECTION_CANCELED, "Operation cancelled by user request.");
                        LOG.error("%s", getLastErrorMsg());
                        break;
                    }
