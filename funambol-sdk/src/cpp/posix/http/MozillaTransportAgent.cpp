...
// ugly hack to prevent inclusion of mozilla internal Strings
// (apparently they're not needed anyway)
...
char* MozillaTransportAgent::sendMessage(const char* msg) 
{
    ...
    LOG.debug("SetRequestHeader User-Agent: %s", userAgent.c_str());

    LOG.debug("Sending HTTP Request: %s", msg);

    nsCString stringMsg(msg);    
    LOG.debug("Sending HTTP Request: %s", stringMsg.get());
    ...
    messageBody->SetAsAUTF8String(stringMsg);
    contentLength = stringMsg.Length();
