    ...
    for (numretries=0; numretries < MAX_RETRIES; numretries++) {
        ...
        if ((iLastHTTPStatus == HTTP_UNAUTHORIZED) || 
            ...
            (iLastHTTPStatus == HTTP_NOT_FOUND)    ||
                // POST is not an idempotent operation: no retry
                // with the same url
                (httpVerb == HTTP::EPOST)) {
            break;         
        }
    ...
    switch ( aEvent.iStatus ) 
    {
        ...
        case THTTPEvent::EGotResponseBodyData:
        {
            ...
            if (length>0 && iOutputStream) {
                if (iOutputStream->write((const char*)ptr.Ptr(), length) < 0) {
                    LOG.error("%s: error writing to data stream: cancelling operation", __FUNCTION__);
