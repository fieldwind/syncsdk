 ...
 */
CSymbianTransportAgent* CSymbianTransportAgent::NewL(const URL& aUrl,Proxy& aProxy,
 ...
 * Creates instance of CSymbianTransportAgent.
 */
CSymbianTransportAgent* CSymbianTransportAgent::NewLC(const URL& aUrl,Proxy& aProxy,
                                                      unsigned int aResponseTimeout,
                                                      unsigned int /* aMaxMsgSize */)
{
    CSymbianTransportAgent* self;
 ...
 */
CSymbianTransportAgent::CSymbianTransportAgent(const URL& aUrl, Proxy& aProxy,
                                               unsigned int aResponseTimeout) : 
 ...
 */
void CSymbianTransportAgent::ConstructL(const URL& /* aUrl */)
{
                                ...
                                const THTTPEvent& aEvent )
{
    ...
    switch ( aEvent.iStatus ) 
    {
        ...
        default:
        {
            ...
            if (aEvent.iStatus < 0) {
                StringBuffer msg = resolveErrorCode(aEvent.iStatus);
                LOG.error("HTTP event status error: %d (%s)", aEvent.iStatus, msg.c_str());
...
char* CSymbianTransportAgent::sendMessage(const char* msg, unsigned int size)
{
    LOG.error("%s: function not implemented!!", __FUNCTION__);
    return NULL;
}
