    ...
    if (iStatus != KErrNone && iStatus != KErrAlreadyExists) {
        ...
        if (iStatus == KErrCancel) {
            ...
            return iLastError;
                 ...
                 (iStatus >= -4159 && iStatus <= - 4200) ) {  // GPRS APN errors (see Symbian OS error codes)
            // no retry!
...
retry:
    ...
    if (iRetryConnection < MAX_RETRY_CONNECTION) {
        ...
        LOG.error("FConnection: %d attempt failed", iRetryConnection);
    }
    return iLastError;
}
...
RArray<HBufC*> FConnection::GetWifiAPNames()
{
    RArray<HBufC*> names;
    names.Reset();
    ...
    CCommsDatabase* commDb = CCommsDatabase::NewL(EDatabaseTypeIAP);
    CleanupStack::PushL(commDb);
    CApSelect* select = CApSelect::NewLC(*commDb, KEApIspTypeAll, EApBearerTypeWLAN, KEApSortUidAscending);
