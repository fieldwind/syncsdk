...
// Sets a int value into propertyMap.
void WinItem::setIntProperty(const wstring propertyName, const int intValue) {
    WCHAR tmpValue[20];
    wsprintf(tmpValue, TEXT("%d"), intValue);
    setProperty(propertyName, tmpValue);
}
...
// Gets a value from propertyMap and convert into integer.
bool WinItem::getIntProperty(const wstring propertyName, int* intValue) {

    wstring propertyValue;
    bool exists = getProperty(propertyName, propertyValue);

    if (exists && propertyValue.length() > 0) {
        swscanf(propertyValue.c_str(), TEXT("%d"), intValue);
        return true;
    }
    return false;
}
