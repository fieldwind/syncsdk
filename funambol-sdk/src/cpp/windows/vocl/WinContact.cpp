...
wstring WinContact::toString() {
    ...
    if (getProperty(L"FileAs", element)) {
        wstring firstName, lastName, companyName;
        bool hasDefaultVal = false, 
             hasPropVal  = false;
        StringBuffer prop;
        int propLen = 0;

        prop.convert(element.c_str());
        propLen = prop.length() - 1;

        // filter out FN default values (such as "<Unnamed>" put
        // by Windows Mobile) which cause duplicates on slow syncs
        // (FN is checked for twins by server when first name, last
        // name and company name are all empty)
        if ((prop[0] == '<') && (prop[propLen] == '>')) {
            hasDefaultVal = true;
           
            if (getProperty(L"FirstName",  firstName)) {
                if (firstName.length()) {
                    hasPropVal = true;
                }
            }

            if (getProperty(L"LastName", lastName)) {
                if (lastName.length()) {
                    hasPropVal = true;
                }
            }
    
            if (getProperty(L"CompanyName", companyName)) {
        ...
        if ((hasDefaultVal == false) || (hasDefaultVal && hasPropVal)) {
...
int WinContact::parse(const wstring & dataString) {
    return parseMapReset(dataString, true);
}
//
// Parse a vCard string and fills the propertyMap.
//
int WinContact::parseMapReset(const wstring & dataString, bool mapReset) {
