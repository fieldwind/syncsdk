...
int DeviceManagementNode::getChildrenMaxCount() {
    ...
    if (key == 0) {
		// If key doesn't exist, report zero children (don't report error)
        goto finally;
    }
...
ArrayElement* DeviceManagementNode::clone() {
	...
	for (int i = 0; i<n; ++i) {
		ret->addChild((ConfigurationNode*)children[i]);
	}
...
StringMap* DeviceManagementNode::readAllProperties() { 
    ...
    TCHAR propertykey[1024] = TEXT("");
    ULONG dim = 1024;
        
    while(ret1 == S_OK) {
        ret1 = RegEnumValue(key, i, propertykey, &dim, NULL, NULL, NULL, NULL); 
        ...
        memset(propertykey, 0, sizeof(TCHAR));
        dim = 1024;
        i++;
    }    
...
int DeviceManagementNode::deleteProperty(const char* propertyName) { 

    int err = 0;

    if (propertyName) {        
        const WCHAR* pname = toWindows(propertyName);

#ifdef _WIN32_WCE
        err = RegDeleteValue(HKEY_DM_ROOT, pname);
#else
        err = SHDeleteValue(HKEY_DM_ROOT, getFullContext(), pname);
#endif

        if (err != ERROR_SUCCESS) {
            setErrorF(ERR_DM_TREE_NOT_AVAILABLE, "Could not delete property: %ls", pname);
        }
        delete [] pname;
    }
    return err;
