    ...
    if (logFile) 
    {
        ...
        if (size>=maxSize*MB)
        {
            if (! fclose(logFile))
            {
                ...
                {
                    newname.sprintf("%s.%d", logDir.c_str(), lastID);
                ...
                {
                    newname.sprintf("%s.%d", logDir.c_str(), t);
                    oldname.sprintf("%s.%d", logDir.c_str(), t-1);

                    renameFile(oldname.c_str(), newname.c_str());
                }
                newname.sprintf("%s.%d", logDir.c_str(), 1);

                remove(newname.c_str());
                renameFile(logDir.c_str(), newname.c_str());

                logFile = fileOpen(logDir.c_str(), "a+");
                if (logFile == NULL && strcmp(logName.c_str(), LOG_NAME)!=0) 
                {
