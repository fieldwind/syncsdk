...
bool PlatformAdapter::slashSeparator = true;

// Initializes the platform-dependent parameters of the library using defaults.
void PlatformAdapter::init(const char *appcontext, bool useSlashSeparator, const bool force) {
    if(!initialized || force) {
        appContext = appcontext;
        homeFolder = "";
        configFolder = "";
        initialized = true;
		slashSeparator = useSlashSeparator;
		if (slashSeparator == true) {
...
void PlatformAdapter::init(const char *appcontext, bool useSlashSeparator, StringMap& env, const bool force) {
    if(!initialized || force) {
        appContext = appcontext;
		...
		if (slashSeparator == true) {
			appContext.replaceAll("\\", "/");
			homeFolder.replaceAll("\\", "/");
			configFolder.replaceAll("\\", "/");
		}
    }
    else {
        LOG.error("PlatformAdapter::init(): already initialized.");
