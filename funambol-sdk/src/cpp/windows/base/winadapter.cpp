...
static void convertSlashToWindows(char* str) {
...
int renameFile(const char* oldFilename, const char* newFilename) {
    ...
    char* t1 = stringdup(oldFilename);
    char* t2 = stringdup(newFilename);
    convertSlashToWindows(t1);
    convertSlashToWindows(t2);

    WString o = L"\\\\?\\";     // see MoveFile api
    WString n = L"\\\\?\\";
    const WCHAR* woldFilename = toWideChar(t1);
    const WCHAR* wnewFilename = toWideChar(t2);

    o.append(woldFilename);
    n.append(wnewFilename);
    ...
    int res = DeleteFile(wnewFilename);
    if (res == 0) {
    ...
    if (ret == 0) {
        if(MoveFile(o.c_str(), n.c_str()) == FALSE) {
                ret = 1;
        }
    }
    delete [] woldFilename;
...
int64_t fileTell(FILE* f) {
    return _ftelli64(f);    
}

int fileSeek(FILE* f, int64_t offset, int whence) {
...
int statFile(const char* filename, struct stat* s) {
    ...
    StringBuffer ss(filename);
    if (ss.endsWith("/") || ss.endsWith("\\")) {
		if (ss.length() > 3) {		// for drive names, like "C:\", we MUST not remove the trailing slash!
			ss = ss.substr(0, ss.length() - 1);
		}
    }

    /*const WCHAR* wfilename = toWideChar(ss.c_str());
    int ret = _wstat(wfilename, (struct _stat64i32 *)s);
    delete [] wfilename;*/

    const WCHAR* wfilename = toWideChar(ss.c_str());
    //int ret = _wstati64(wfilename, (struct __stat64*)s); 
    int ret = _wstati64(wfilename, s); 
    delete [] wfilename;
...
int getFreeDiskSpace(const char* path, unsigned long long *totalBytes, unsigned long long *freeBytes) {
    ...
    if (path && strlen(path)>0) {
        wdir = toWideChar(path);
    }
    BOOL res = GetDiskFreeSpaceEx(wdir, &free, &total, &totalFree);
    //delete wdir;

    if (res == FALSE) {
        DWORD error = GetLastError();
        LOG.error("GetDiskFreeSpaceEx() failed: path=%s, windows code=%ul", path? path:"(NULL)", error);
        if (error == ERROR_PATH_NOT_FOUND) {
			// Try to create it
			if (CreateDirectory(wdir, NULL) != 0) { // success
                res = GetDiskFreeSpaceEx(wdir, &free, &total, &totalFree);
                if (res == FALSE) {
                    ret = ERR_FILE_SYSTEM;
                }
            } else {
...
unsigned long getFileModTime(const char* name) {
	struct stat buffer;
	return statFile(name, &buffer) ? 0 : (unsigned long)buffer.st_mtime;
}

unsigned long getFileCreationDate(const struct stat& st)
{
    return st.st_ctime;
}
...
ArrayList readFilesInDirRecursive(const char* dirname, bool recursive) {
    ...
    DWORD dwAttrs;
    WCHAR toFind    [1024];
    WCHAR szNewPath [1024];
    bool fFinished = false;

    WCHAR* wname = toWideChar(dirname);
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        LOG.error("Invalid handle for retrieve files from %s", dirname);
        goto finally;
    }
    else {
        while (!fFinished) {
            ...
            if ((dwAttrs & FILE_ATTRIBUTE_DIRECTORY)) {
                if (recursive) {
                    StringBuffer element; element.convert(FileData.cFileName);
                    if (element != "." && element != "..") {
                        ArrayList list;
                        ...
                        newDir.append(element);
                        list = readFilesInDirRecursive(newDir.c_str(), recursive);
                        totalFiles.add(&list);
                    }
                }

            } else if ((dwAttrs & FILE_ATTRIBUTE_HIDDEN) || (dwAttrs & FILE_ATTRIBUTE_SYSTEM) ) {
                // do nothing
            } else {
                StringBuffer element; element.convert(FileData.cFileName);
...
ArrayList readFilesInDirRecursive(const char* dirname, bool recursive, const std::vector<std::string>& filterPatterns, std::map<std::string, bool>& readFoldersMap) {
     ArrayList totalFiles;
    WIN32_FIND_DATA FileData;
    HANDLE hFind;
    DWORD dwAttrs;
    ...
    StringBuffer dd(dirname);
    if (dd.endsWith("/") || dd.endsWith("\\")) {
        dd = dd.substr(0, dd.length() - 1);        
    }

    WCHAR* wname = toWideChar(dd.c_str());
    wsprintf(toFind, TEXT("%s\\*.*"), wname);
    ...
    hFind = FindFirstFile(toFind, &FileData);
    if (hFind == INVALID_HANDLE_VALUE) {
        LOG.error("Invalid handle for retrieve files from %s", dd.c_str());
        goto finally;
    }
    else {
        while (!fFinished) {
            ...
            if ((dwAttrs & FILE_ATTRIBUTE_DIRECTORY)) {
                if (recursive) {
                    ...
                    if (element != "." && element != "..") {
                        ArrayList list;
                        StringBuffer newDir(dd.c_str());                        
                        newDir.append("/");
                        newDir.append(element);
                        std::pair<std::string, bool> kv(newDir.c_str(), true);
                        
                        readFoldersMap.insert(kv);

                        if (applyFilterFromPatternsList(element.c_str(), filterPatterns) == false) {
                            list = readFilesInDirRecursive(newDir.c_str(), recursive, filterPatterns, readFoldersMap);
                            totalFiles.add(&list);
                        } 
                        // newDir.append(element);
                        // list = readFilesInDirRecursive(newDir.c_str(), recursive);
...
bool removeFileInDir(const char* d, const char* fname) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        while (!fFinished) {
            ...
            if ( wcscmp(FileData.cFileName, TEXT(".")) &&
                 wcscmp(FileData.cFileName, TEXT("..")) ) {

                wsprintf(szNewPath, TEXT("%s/%s"), dir, FileData.cFileName);
                dwAttrs = GetFileAttributes(szNewPath);
                if (dwAttrs == FILE_ATTRIBUTE_DIRECTORY) {
                    // do nothing for subdirectory
                }
                else {
                    if (DeleteFile(szNewPath) == FALSE) {
                        LOG.error("DeleteFile error for file %ls (code %d)", szNewPath, GetLastError());
                        ret = false;
                    }
                }
            }
