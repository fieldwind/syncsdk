...
bool FSyncConfig::read() {
	...
	if (val) {
	    syncPath = val; delete [] val;
	}

        val = node->readPropertyValue(FSYNC_CACHEDIR_PROPERTY);
	if (val) {
	    cacheDir = val; delete [] val;
	}
...
bool FSyncConfig::save() {
    ...
    if (node) {
        ...
        node->setPropertyValue(FSYNC_CACHEDIR_PROPERTY, cacheDir);
...
void FSyncConfig::createConfig() {
    ...
    cacheDir = rootContext; cacheDir.append(FSYNC_CACHEDIR);
...
/**
 * Set the the cache dir ued for media sync. If the new path is different from the current one,
 * set the SyncSource last sync to 0, in order to force a slow sync.
 *
 * @param newCacheDir   the new cacheDir
 */
void FSyncConfig::setCacheDir(const char *newCacheDir) { 
