...
#define TEST_NAME           "configSyncSourceIntegration"
#define CONTEXT             PlatformAdapter::getAppContext().c_str()    // defined at beginning calling getConf()
...
void setpropertyconfig(const char* prop, const char* value){
    ...
    node->setPropertyValue(value, "");          // all empty
    delete node;
}

void ConfigSyncSourceTest::testConfigSource() {
   // create the first configuration
    DMTClientConfig* config1 = getConf(TEST_NAME);
