...
private:
    ...
    void testProcessServerDevInf1() {
        ...
        bVal  = config.getServerMediaHttpUpload();CPPUNIT_ASSERT(bVal == true);

        // Check datastores: 3 expected (card, event, picture)
        const ArrayList* dataStores = config.getServerDataStores();
        CPPUNIT_ASSERT(dataStores);
        CPPUNIT_ASSERT(dataStores->size() == 3);

        DataStore* dataStore = (DataStore*)dataStores->get(0);
        checkDataStore(dataStore, "card", "card", 32, "text/x-vcard", "2.1", "text/x-vcard", "2.1");

        dataStore = (DataStore*)dataStores->get(1);
        checkDataStore(dataStore, "event", "event", 32, "text/x-vcalendar", "1.0", "text/x-vcalendar", "1.0");

        dataStore = (DataStore*)dataStores->get(2);
        checkDataStore(dataStore, "picture", "picture", 32, OMA_MIME_TYPE, "1.0", OMA_MIME_TYPE, "1.0");
    }
    ...
    void testProcessServerDevInf2() {
        ...
        bVal  = config.getServerMediaHttpUpload();CPPUNIT_ASSERT(bVal == false);
        ...
        DataStore* dataStore = (DataStore*)dataStores->get(0);
        checkDataStore(dataStore, "./contacts", "Addressbook", 0, "text/x-vcard", "2.1", "text/x-vcard", "2.1");
    }


    /// Checks the content of the passed DataStore object. Other params are the expected values.
    void checkDataStore(DataStore* dataStore,     const char* sourceRef, 
                        const char*  displayName, const long maxGUIDSize, 
                        const char* rxPrefType,   const char* rxPrefVersion, 
                        const char* txPrefType,   const char* txPrefVersion ) {
    
        CPPUNIT_ASSERT(dataStore);

        long lVal = 0;
        StringBuffer value;
        value = dataStore->getSourceRef()->getValue();      CPPUNIT_ASSERT(value == sourceRef);
        value = dataStore->getDisplayName();                CPPUNIT_ASSERT(value == displayName);
        lVal  = dataStore->getMaxGUIDSize();                CPPUNIT_ASSERT(lVal  == maxGUIDSize);
        value = dataStore->getRxPref()->getCTType();        CPPUNIT_ASSERT(value == rxPrefType);
        value = dataStore->getRxPref()->getVerCT();         CPPUNIT_ASSERT(value == rxPrefVersion);
        value = dataStore->getTxPref()->getCTType();        CPPUNIT_ASSERT(value == txPrefType);
        value = dataStore->getTxPref()->getVerCT();         CPPUNIT_ASSERT(value == txPrefVersion);

        // rx and tx are not parsed
        // syncTypes are parsed but not used
    }
