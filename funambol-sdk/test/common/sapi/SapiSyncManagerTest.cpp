...
public:
    ...
    int beginSync(bool changes, AbstractSyncConfig& mainConfig) {
        ...
        for (int i=0; i<delCount; i++) {
            ...
            SapiSyncItemInfo itemInfo(luid.c_str(), luid.c_str(), luid.c_str(), i+4000, "", "", "", 0, i+4000);
...
public:
    ...
    ESapiMediaRequestStatus deleteItem(SapiSyncItemInfo* itemInfo) {
        CPPUNIT_ASSERT (itemInfo != NULL);
        return ESMRSuccess;
    }
...
private:
    ...
    void testLogin() {
        ...
        CPPUNIT_ASSERT (err == ESSMSapiError);
    ...
    void testUploadChanges() {
        ...
        source->setModCount(numUploads); 
        source->setDelCount(numUploads); 

        checkUploadChanges(manager, *source, numUploads*3, numUploads);
        cleanup(*source);
        delete source;
    }
    ...
    void checkUploadChanges(TestSapiSyncManager& manager, TestSapiSyncSource& source, const int expectedUploads, const int expectedAdd,
                     ESapiSyncManagerError expectedRet = ESSMSuccess) {
        ...
        Enumeration& mappings = source.getMappings().getProperties();
        Enumeration& cache = source.getCache().getProperties();
        for (int i=0; i<expectedUploads - 6; i++) {
            CPPUNIT_ASSERT (mappings.getNextElement() != NULL);
            CPPUNIT_ASSERT (cache.getNextElement() != NULL);
        }
        CPPUNIT_ASSERT (mappings.getNextElement() == NULL);
        ...
        SyncSourceReport* ssr = report.getSyncSourceReport(TEST_SOURCE_NAME);
        CPPUNIT_ASSERT (ssr != NULL);
        CPPUNIT_ASSERT_EQUAL (expectedAdd, ssr->getItemReportCount(SERVER, COMMAND_ADD));
        CPPUNIT_ASSERT_EQUAL (expectedAdd, ssr->getItemReportCount(SERVER, COMMAND_DELETE));
        CPPUNIT_ASSERT_EQUAL (expectedAdd, ssr->getItemReportCount(SERVER, COMMAND_REPLACE));
