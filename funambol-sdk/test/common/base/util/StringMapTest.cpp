...
#define TESTVAL "testvalue"
    ...
    void testPut() {
        sm.clear();
        bool ret = sm.put(NULL, "value");
        CPPUNIT_ASSERT(ret == false);
        ret = sm.put("key", NULL);
        CPPUNIT_ASSERT(ret == false);
        CPPUNIT_ASSERT(sm.empty() == true);
    }
    ...
    /* Test the iteration over the map */
    void testIteration() {
        KeyValuePair kvp = sm.front();
        CPPUNIT_ASSERT(kvp == KeyValuePair("key1", "value1"));

        kvp = sm.next();
        CPPUNIT_ASSERT(kvp == KeyValuePair("key2", "value2"));

        kvp = sm.next();
        CPPUNIT_ASSERT(kvp == KeyValuePair("key3", "value3"));

        kvp = sm.next();
        CPPUNIT_ASSERT(kvp.null());
    ...
    /* Test the iteration over an empty map */
    void testEmptyFront() {
        sm.clear();
        KeyValuePair front = sm.front();
        CPPUNIT_ASSERT(front.null());
    }
