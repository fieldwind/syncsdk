...
void InputStreamTest::testReadTwoChunks(InputStream& stream, const void* expectedData, const int expectedDataSize) {
    ...
    CPPUNIT_ASSERT (memcmp(outBuffer, (const char *)expectedData + firstChunkSize, secondChunkSize) == 0);
    CPPUNIT_ASSERT (stream.getPosition() == expectedDataSize);
    CPPUNIT_ASSERT (stream.eof());
...
void InputStreamTest::testReadManyChunks(InputStream& stream, const void* expectedData, const int expectedDataSize, const int chunkSize) {
    ...
    while (!stream.eof()) {
        ...
        CPPUNIT_ASSERT (memcmp(outBuffer, (const char *)expectedData + offset, bytesRead) == 0);
    }
...
/// Test the set/getPosition inside the stream
void InputStreamTest::testStreamPosition(InputStream& stream, const void* expectedData, const int offset, const int size) {

    stream.setPosition(offset);
    CPPUNIT_ASSERT (stream.getPosition() == offset);

    // safe checks
    if (stream.eof()) return;
    if (offset + size > (int)stream.getTotalSize()) return;

    // read 'size' bytes from the offset position
    char* outBuffer = new char[size + 1];
    memset(outBuffer, '-', size);
    int bytesRead = stream.read(outBuffer, size);

    CPPUNIT_ASSERT (bytesRead == size);
    CPPUNIT_ASSERT (memcmp(outBuffer, (const char *)expectedData+offset, size) == 0);
    CPPUNIT_ASSERT (stream.getPosition() == offset+size);
