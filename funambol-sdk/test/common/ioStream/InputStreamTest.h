...
public:
    ...
    /**
     * Tests the set/getPosition and reads the stream
     * @param stream           the InputStream to read from (expected already initialized)
     * @param expectedData     the data expected to read
     * @param offset           the offset to set the pointer to
     * @param size             the number of bytes to read at that offset
     */
    void testStreamPosition(InputStream& stream, const void* expectedData, const int offset, const int size);
